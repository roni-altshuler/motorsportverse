...
export const metadata: Metadata = {
  title: "Calendar — RaceIQ IMSA",
  description:
    "The IMSA WeatherTech SportsCar Championship season calendar — every round from the sprint races to the Rolex 24 At Daytona, with completed and upcoming status.",
};
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <header className="mb-8">
        ...
        <p className="body-md mt-3 max-w-2xl text-[color:var(--muted)]">
          The season runs from short sprint rounds to the Michelin Endurance Cup classics — the Rolex
          24 At Daytona, the 12 Hours of Sebring and beyond. Completed rounds link through to their
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {data.calendar.map((r) => (
          <Link key={r.round} href={`/round/${r.round}`}>
            <Card
              interactive
              className={`p-5 h-full ${r.isEnduranceCup ? "podium-leader-card" : ""}`}
            >
              {r.isEnduranceCup && <div className="podium-leader-accent" />}
              ...
              {r.isEnduranceCup && (
                <p className="mt-4 pt-3 border-t border-[color:var(--hairline)] body-sm text-[color:var(--muted)]">
                  A Michelin Endurance Cup round — a longer race worth extra points and a sterner
