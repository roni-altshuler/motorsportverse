...
// The data-root constant lives here (safe to import from client components).
 ...
 * index derived from imsa.json when the index is missing, so older deployments
 * keep working.
 */
export async function fetchSeasonsIndex(): Promise<SeasonsIndex> {
  ...
  try {
    const res = await fetch(`${BASE_PATH}/imsa.json`);
    if (res.ok) {
