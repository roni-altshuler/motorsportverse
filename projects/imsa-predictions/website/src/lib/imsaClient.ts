...
// Client-side loaders for the IMSA dataset. The Navbar and other client
...
import type { ProbabilitiesRound, RoundDetail, ImsaData } from "@/types/imsa";

import { useSeason } from "./SeasonProvider";
import { BASE_PATH } from "./seasons";

const cache = new Map<string, ImsaData | null>();
const inflight = new Map<string, Promise<ImsaData | null>>();

export function fetchImsaData(base: string = BASE_PATH): Promise<ImsaData | null> {
  if (cache.has(base)) return Promise.resolve(cache.get(base) ?? null);
  const pending = inflight.get(base);
  if (pending) return pending;
  const p = fetch(`${base}/imsa.json`)
    .then((r) => (r.ok ? (r.json() as Promise<ImsaData>) : null))
    .then((d) => {
      cache.set(base, d);
...
/** imsa.json for a data root (defaults to the CURRENT season's root). */
export function useImsaData(base: string = BASE_PATH): ImsaData | null {
  const [data, setData] = useState<ImsaData | null>(cache.get(base) ?? null);
  useEffect(() => {
    let active = true;
    fetchImsaData(base).then((d) => {
      if (active) setData(d);
    });
    return () => {
      active = false;
...
/**
 * imsa.json for the season selected in the SeasonProvider. `isArchived` is true
 * only when a non-current season is selected.
 */
export function useSeasonImsaData(): { data: ImsaData | null; isArchived: boolean } {
  const { basePath, year, index } = useSeason();
  const data = useImsaData(basePath);
