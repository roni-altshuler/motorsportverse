"""The IMSA website data contract: imsa.json + per-round probability files.

Everything the site reads is keyed by class (IMSA is scored per class), so the
export carries per-class standings / championship / next-round prediction and a
per-class + overall season-accuracy block. Kept fast by reducing the Monte-Carlo
...
from imsa_predictions import config, export
...
def data_dir(tmp_path_factory):
    ...
    try:
        out = tmp_path_factory.mktemp("imsadata")
        export.write(out)
    finally:
...
def test_imsa_json_top_level_shape(data_dir):
    imsa = _load(data_dir / "imsa.json")
    assert imsa["sport"] == config.SPORT
    assert isinstance(imsa["classes"], list) and imsa["classes"], "no classes exported"
    class_keys = [c["key"] for c in imsa["classes"]]
    for c in imsa["classes"]:
        assert {"key", "label", "color"} <= set(c)

    # per-class standings + championship for every regular-season class
    assert set(imsa["standings"]) == set(class_keys)
    assert set(imsa["championship"]) == set(class_keys)
    for cls in class_keys:
        for row in imsa["standings"][cls]:
            assert {"position", "code", "points", "wins", "podiums"} <= set(row)
        champ = imsa["championship"][cls]
...
def test_probability_files_win_market_sums_to_one(data_dir):
    ...
    for path in files:
        ...
        for cls in payload["classes"]:
            ...
            assert abs(total - 1.0) < 0.02, f"{path.name}/{cls.get('key')} win market sum={total:.3f}"
