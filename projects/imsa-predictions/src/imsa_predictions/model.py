"""The IMSA WeatherTech prediction model — per class.

IMSA is multi-class and car-based, so the model forecasts each class as its own
field and the competitor is the **car entry**. For a class at a round it estimates
a leakage-safe latent *pace* per entry (lower = faster) by blending:

* **cross-season entry Elo** — the car's own strength, carried year to year
  (top entries keep their number, so ``GTP-7`` is Porsche Penske #7 across seasons);
* **this-season form** — average within-class finishing position so far, with a
  prior-season fallback so the opener isn't blind;
* **team strength** and **manufacturer strength** — a factory Porsche/Cadillac/
  Acura/BMW is a different weapon from a customer car, and endurance rewards the
...
def _elo_ratings(source: ImsaDataSource, year: int, round: int, cls: str,
...
def _lastrace_signal(source: ImsaDataSource, year: int, round: int, cls: str,
...
def estimate_class_skill(source: ImsaDataSource, year: int, round: int, cls: str,
                         field: list[str]) -> dict[str, float]:
    ...
    leakage.assert_prior_only(
        {r: None for r in prior}, current_round=round, label=f"imsa.model.skill.{cls}"
    )
    if not field:
        return {}
...
def forecast_class(source: ImsaDataSource, year: int, round: int, cls: str, *,
...
def forecast_round(source: ImsaDataSource, year: int, round: int, *,
