"""Forward-time evaluation for the IMSA WeatherTech Championship — leakage-safe,
per class.

**This is the headline honesty surface (the never-ship-worse gate) for the IMSA
model.** IMSA is multi-class (GTP/DPi, LMP2, GTD PRO, GTD, plus LMP3 in the
2022-2023 era), so a round is not one race but one *per class*. For every completed
round and every class that ran it, we re-run the model with **only prior-round
data** and score its within-class markets against the actual within-class
classification, then publish the same numbers for two trivial baselines so "is the
model beating the obvious predictor?" is a single read:
...
Scoring is over the **classified** entries in each class (retirements are
unranked). Metrics reuse :mod:`motorsport_core.eval` (``score_round`` bundle +
...
Run:  python -m imsa_predictions.forward_eval --season 2026 [--allow-empty] [--out <dir>]
"""
...
# differs, so the Brier comparison isolates the signal.
...
def _lastrace_signal(source: ImsaDataSource, year: int, rnd: int, cls: str,
...
def _form_signal(source: ImsaDataSource, year: int, rnd: int, cls: str,
                 field: list[str]) -> dict[str, float]:
...
def _baseline_block(source: ImsaDataSource, year: int, rnd: int, cls: str,
...
# --------------------------------------------------------------------------- #
def evaluate_season(source: ImsaDataSource, year: int) -> list[dict]:
