"""Generate the IMSA WeatherTech website's JSON from the model + committed snapshots.
...
because IMSA is scored per class (GTP / LMP2 / GTD PRO / GTD, plus DPi/LMP3 in
older seasons):

    public/data/
      imsa.json                    season summary: calendar, per-class standings,
...
Only classes actually racing at a round are forecast (endurance rounds add LMP2;
street sprints often don't). Standings and championship cover the **regular-season
classes** only — the classes that contest the full-season title.

Run:  python -m imsa_predictions.export   [--out <dir>]
"""
...
# A full IMSA WeatherTech season has run 11-12 rounds in recent years; when the
# live season's remaining calendar isn't yet in the archive we project the title
# assuming it reaches this many rounds (documented in the payload's ``basis``).
EXPECTED_TOTAL_ROUNDS = 11
H2H_TOP_N = 10
_NEUTRAL = "#8A8A8A"
...
def _regular_classes(source: ImsaDataSource, year: int) -> list[str]:
    """Classes that contest the full-season title (exclude one-off / limited classes)."""
...
# --------------------------------------------------------------------------- #
def _class_standings(source: ImsaDataSource, year: int, cls: str) -> list[dict]:
...
# --------------------------------------------------------------------------- #
def _class_championship(source: ImsaDataSource, year: int, cls: str,
...
# Per-round detail
# --------------------------------------------------------------------------- #
def _actual_class(source: ImsaDataSource, year: int, rnd: int, cls: str) -> dict[str, int]:
    res = source.class_results(year, rnd, cls) or []
...
def _class_block(cf: ClassForecast, source: ImsaDataSource, year: int, rnd: int,
                 completed: bool) -> dict:
...
def round_payload(fc: RoundForecast, source: ImsaDataSource, completed: bool) -> dict:
    return {
        "round": fc.round,
        "season": fc.season,
        "place": fc.place,
...
def _calibrate_markets(cf: ClassForecast, calibrator) -> dict:
    ...
    for market in _RAW_MARKET_KEYS:
        ...
        if calibrator is not None and calibrator.is_fitted(market, cf.cls):
            ...
            cal_vals = raw_vals
        # The win market is mutually exclusive (exactly one class winner), so its
        # calibrated probabilities must sum to 1.0. Per-market isotonic calibration
        # plus the probability floor over the class field inflates the sum, so
        # renormalise WIN per class. Top-k markets are independent per-entry
        ...
        out[market] = {
            c: {"probability": round(float(cal_vals[i]), 4),
...
def build_calibrator(round_forecasts: dict[int, RoundForecast], source: ImsaDataSource, year: int):
...
def _calibration_summary(calibrator, real_rounds: int) -> dict:
    ...
    return {
        ...
        "dataLimitation": (
            "Calibrated on real IMSA results, per class stratum." if applied
...
# --------------------------------------------------------------------------- #
def _season_accuracy(round_forecasts: dict[int, RoundForecast], source: ImsaDataSource,
                     year: int) -> dict:
...
# --------------------------------------------------------------------------- #
# Top-level season summary (imsa.json)
# --------------------------------------------------------------------------- #
def build_payload(round_forecasts: dict[int, RoundForecast], source: ImsaDataSource,
        ...
        "calendar": [
            {
                ...
                "isEnduranceCup": e.get("isEnduranceCup", False),
