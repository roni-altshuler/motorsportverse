"""IMSA data source — per-class results + leakage-safe history for the model.

Wraps :class:`imsa_predictions.sources.snapshot.SnapshotImsaSource` and adds the
...
from . import config
from .sources.snapshot import SnapshotImsaSource


class ImsaDataSource:
    sport = config.SPORT

    def __init__(self) -> None:
        self.snapshot = SnapshotImsaSource()

    # ---- direct results ----------------------------------------------------
    def class_results(self, year: int, round: int, cls: str):
    ...
    def classes_for_round(self, year: int, round: int) -> list[str]:
        ...
        A completed round reports the classes it ran (endurance rounds add LMP2;
        street sprints often don't). For a not-yet-run round we use the most
        recent completed round's classes — the regular grid — so we never forecast
        a class that won't be there. Falls back to the season's class list.
        """
...
# Backwards-compatible alias (the scaffold exported ``SportDataSource``).
SportDataSource = ImsaDataSource
