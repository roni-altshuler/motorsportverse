 ...
 * index derived from nascar.json when the site predates multi-season support,
...
export async function fetchSeasonsIndex(): Promise<SeasonsIndex> {
  ...
  // Legacy fallback: derive the current season from nascar.json.
  let current = new Date().getFullYear();
  try {
    const res = await fetch(`${BASE_PATH}/nascar.json`);
    if (res.ok) {
