 ...
 * a NASCAR Cup race — or the whole season — to Apple Calendar, Google Calendar,
 ...
 * server and `"use client"` code in a static export.
 *
 * The season data (nascar.json calendar entries) gives us a race *date* but no
 * confirmed green-flag *time*, so by default each race is written as an all-day
 * VEVENT on race day (DTSTART;VALUE=DATE). When a real start time is known it
 * can be passed via `startUtc` and a timed, 3-hour UTC event is emitted instead
...
/** Minimal shape of a race, matching nascar.json `calendar[*]` entries. */
export interface CalendarRace {
  ...
  /** e.g. "United States" */
  country?: string;
  /** True for the ten Chase (playoff) rounds. */
  isPlayoff?: boolean;
...
export interface IcsOptions {
  ...
  now?: Date;
  /** Duration in minutes for timed events (only used when `startUtc` is set). Default 180. */
  timedDurationMinutes?: number;
}

const PRODID = "-//RaceIQ//NASCAR Cup Race Calendar//EN";
...
/** "2026-02-15" -> "20260215" (date-only, floating). */
...
function stableUid(race: CalendarRace, season?: number): string {
  ...
  return `nascar-${yr}-round-${race.round}@raceiq`;
}
...
function buildVeventLines(race: CalendarRace, opts: IcsOptions): string[] {
  ...
  const descParts = [
    `Round ${race.round} of the${opts.season ? " " + opts.season : ""} NASCAR Cup Series season.`,
  ];
  if (race.circuit) descParts.push(`Track: ${race.circuit}.`);
  if (race.isPlayoff) descParts.push("Chase playoff race.");
  descParts.push("Predictions & standings at RaceIQ NASCAR.");
  ...
  if (race.startUtc) {
    ...
    const durMs = (opts.timedDurationMinutes ?? 180) * 60 * 1000;
  ...
  lines.push(`SUMMARY:${escapeText(`${race.name} — NASCAR Cup`)}`);
...
export function buildRaceIcs(race: CalendarRace, opts: IcsOptions = {}): string {
  const name = `${race.name}${opts.season ? ` (NASCAR ${opts.season})` : " (NASCAR)"}`;
...
export function buildSeasonIcs(
  ...
  const name = `NASCAR Cup${opts.season ? ` ${opts.season}` : ""} Season — RaceIQ`;
...
export function icsFilename(race?: CalendarRace, season?: number): string {
  if (!race) return `nascar-${season ?? "season"}-calendar.ics`;
  const slug = race.name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/(^-|-$)/g, "");
  return `${slug || `round-${race.round}`}-nascar${season ? `-${season}` : ""}.ics`;
}
