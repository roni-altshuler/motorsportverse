...
import type { NascarData, ProbabilitiesRound, RoundDetail } from "@/types/nascar";

import { useSeason } from "./SeasonProvider";
import { BASE_PATH } from "./seasons";

const cache = new Map<string, NascarData | null>();
const inflight = new Map<string, Promise<NascarData | null>>();

export function fetchNascarData(base: string = BASE_PATH): Promise<NascarData | null> {
  if (cache.has(base)) return Promise.resolve(cache.get(base) ?? null);
  const pending = inflight.get(base);
  if (pending) return pending;
  const p = fetch(`${base}/nascar.json`)
    .then((r) => (r.ok ? (r.json() as Promise<NascarData>) : null))
    .then((d) => {
      cache.set(base, d);
...
/** nascar.json for a data root (defaults to the CURRENT season's root). */
export function useNascarData(base: string = BASE_PATH): NascarData | null {
  const [data, setData] = useState<NascarData | null>(cache.get(base) ?? null);
  useEffect(() => {
    let active = true;
    fetchNascarData(base).then((d) => {
      if (active) setData(d);
    });
    return () => {
      active = false;
...
/**
 * nascar.json for the season selected in the SeasonProvider. `isArchived` is true
 ...
 */
export function useSeasonNascarData(): { data: NascarData | null; isArchived: boolean } {
  const { basePath, year, index } = useSeason();
  const data = useNascarData(basePath);
