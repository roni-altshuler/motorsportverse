...
import ChasePanel from "@/components/standings/ChasePanel";
import ManufacturerPanel from "@/components/standings/ManufacturerPanel";
...
export interface StandingsPageProps {
  ...
  /** Chase projection — passed ONLY when the playoff backtest gate passed. */
  playoffProjection?: PlayoffProjection | null;
}
...
// season's nascar.json (client-side). Uses each row's exported pointsHistory —
// the same fallback the server page's series builder uses.
function propsFromSeasonData(data: NascarData): StandingsPageProps {
  const TOP_DRIVERS = 6;
  const TOP_TEAMS = 5;
  ...
  return {
    ...
    // playoff_projection.json is a current-season artefact — never overlay it
    // onto an archived season.
...
function StandingsPageInner(baked: StandingsPageProps) {
  ...
  const { data: seasonData, isArchived } = useSeasonNascarData();
  const {
    season,
    completedRounds,
    totalRounds,
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 section-bugatti">
      <div className="mb-16 text-center">
        <p className="eyebrow mb-4">NASCAR Cup Series · Championship</p>
        <h1 className="display-xl mb-4">{season} Standings</h1>
        <p className="body-md text-[color:var(--body)] max-w-xl mx-auto">
          Who is winning the Cup — and how the playoff picture is shaping up on the
          road to the Chase.
        </p>
        ...
        <div className="mt-6 flex justify-center">
          <ShareButton
            title={`NASCAR Cup ${season} Championship Standings`}
            text={`NASCAR Cup ${season} championship standings & Chase playoff forecast`}
          />
        </div>
      </div>
      ...
      <div className="flex justify-center gap-2 mb-10 relative">
        {(
          [
            ...
            { key: "wdc" as Tab, label: "The Chase", href: "/standings?tab=wdc" },
          ] as const
        ).map((tab) => (
          <a
            key={tab.key}
            ...
            <div className="card p-6">
              <div className="flex items-baseline justify-between gap-3 flex-wrap">
                ...
                <p className="text-[11px] font-mono uppercase tracking-[0.14em] text-[color:var(--text-muted)]">
                  projection to end of regular season
                </p>
              </div>
              <ProgressionChart
                series={driverSeries}
      ...
      {activeTab === "teams" && (
        <div className="space-y-8">
          ...
          {manufacturers && manufacturers.length > 0 && (
            <ManufacturerPanel manufacturers={manufacturers} />
          )}
      ...
      {/* The Chase tab — playoff projection (gated) + regular-season forecast */}
      {activeTab === "wdc" && (
        <div className="space-y-10">
          {playoffProjection && <ChasePanel projection={playoffProjection} />}
          {rounds.length > 0 && driverSeries.length > 0 && (
            <div className="card p-6">
              <div className="flex items-baseline justify-between gap-3 flex-wrap">
              ...
              return (
                <tr key={d.code} style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  <td className="px-2 py-2">
                    <Link href={`/driver/${d.code}`} aria-label={`${d.name} profile`}>
                      <DriverPortrait
                        driver={d.code}
                        driverFullName={d.name}
                        team={d.team}
                  ...
                  <td className="px-4 py-3">
                    <Link
                      href={`/driver/${d.code}`}
                      className="group inline-flex items-baseline hover:underline underline-offset-4"
                    >
                      <span className="font-bold text-[color:var(--ink)]">{d.code}</span>
                      <span className="ml-2 text-xs hidden sm:inline text-[color:var(--text-muted)] group-hover:text-[color:var(--ink)]">
                        {d.name}
                      </span>
                    </Link>
                  </td>
