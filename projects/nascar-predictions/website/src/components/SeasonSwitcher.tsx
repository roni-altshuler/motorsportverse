 ...
 * NASCAR's victory-lane yellow in tokens.css — do not "fix" the variable name.
 */
export default function SeasonSwitcher() {
  const { year, index, hasMultiple, setYear } = useSeason();

  // Prefer the index label ("2026").
  ...
  if (!hasMultiple) {
    return (
      <span
        ...
        title={`NASCAR Cup ${labelFor(year)} season`}
      >
        <span
