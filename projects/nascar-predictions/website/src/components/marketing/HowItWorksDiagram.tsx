 ...
 * RaceIQ F1. The Cup snapshot has no telemetry or weather feeds, so the copy
 * is reworded around what the championship actually provides: per-round race
 * results with stages, a track-type-aware driver model with a DNF hazard, and
 * per-race probabilities. Two
...
const STEPS: Step[] = [
  {
    Icon: Database,
    title: "NASCAR Cup results",
    copy: "Race finishing orders, stage results, grids and championship standings ingested after every checkered flag.",
  },
  {
    Icon: Cpu,
    title: "The model",
    copy: "A track-type-aware driver model — superspeedways, intermediates, short tracks and road courses reward different things, and the forecast is tuned separately for each. A per-driver retirement hazard rides along, and it re-trains as the season unfolds.",
  },
  {
    Icon: Trophy,
    title: "Race forecast",
    copy: "Probabilities for win, podium, top six and top ten for every driver — calibrated on the season's real results.",
  },
];
  ...
  return (
    ...
    >
      ...
      <AnimatedBeam
        ...
        toRef={modelRef}
        duration={4}
        gradientStartColor="#FFD659"
        gradientStopColor="#E9BC2F"
      />
      <AnimatedBeam
        containerRef={diagramRef}
        fromRef={modelRef}
        ...
        duration={4}
        delay={1}
        gradientStartColor="#E9BC2F"
        gradientStopColor="#FFDF80"
      />
    </div>
  );
}
