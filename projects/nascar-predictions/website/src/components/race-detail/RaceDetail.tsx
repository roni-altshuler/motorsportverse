...
import StageResultsPanel from "@/components/race-detail/StageResultsPanel";
...
import { fetchNascarData, fetchRoundDetail, fetchRoundProbabilities } from "@/lib/nascarclient";
...
export function RaceDetail({
  ...
  /** Season calendar — used for the playoff marker on Chase rounds. */
  ...
  // NASCAR scores ONE points race per round — no sprint/feature tabs.
  const block = round.race;
  const probs = probabilities?.race ?? null;
  const raceLabel = "race";
  // Playoff marker lives on the calendar (the ten Chase rounds).
  const calEntry = calendar.find((c) => c.round === round.round);
  const isPlayoff = calEntry?.isPlayoff ?? false;
  const openEntry: ClassificationEntry | null =
    openDriver != null ? block.classification.find((e) => e.code === openDriver) ?? null : null;

  return (
    <div>
      {/* Telemetry-framed header (round / venue / status + track type + stages). */}
      <HUDHeader
        round={round.round}
        ...
        trackType={round.trackType}
        stageLaps={round.stageLaps}
        isPlayoff={isPlayoff}
      ...
      {/* Stage results — the top ten of each completed stage and their points. */}
      {round.completed && block.stageResults && (
        <StageResultsPanel
          stageResults={block.stageResults}
      ...
      <div className="mt-8">
                ...
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      <div className="metric-card">
                        <p className="eyebrow mb-1">Track Type</p>
                        <p className="title-md">{trackTypeLabel(round.trackType)}</p>
                      </div>
                    </>
                  );
                })()}
        ...
        {/* Track & Telemetry — framed track map (no public lap telemetry in the snapshot). */}
        <details className="deep-dive-section">
          <summary className="deep-dive-summary">Track &amp; Telemetry</summary>
          <div className="deep-dive-section-body">
            <motion.div
              initial={reduced ? false : { opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <TrackMapWithOverlay
                geometry={geometry}
                kicker={trackTypeLabel(round.trackType)}
                title={round.venueName}
                className=""
              />
              <p className="body-sm mt-4 text-[color:var(--muted)]">
                {`${trackTypeLabel(round.trackType)} racing: ${TRACK_TYPE_BLURB[round.trackType]}.`}{" "}
                Per-lap telemetry (speed traps, throttle traces) isn&rsquo;t part of our
                results snapshot, so this round focuses on the forecast itself.
              </p>
            </motion.div>
          </div>
        </details>
...
/** DNF hazard indicator — a risk dot + percent, tiered by hazard level. */
function DnfRisk({ p }: { p: number | undefined }) {
  if (typeof p !== "number") return <span className="text-[var(--ink-dim)]">—</span>;
  const tier = p >= 0.18 ? "high" : p >= 0.12 ? "med" : "low";
  const color =
    tier === "high" ? "var(--warning)" : tier === "med" ? "var(--muted)" : "var(--success)";
  return (
    <span className="inline-flex items-center gap-1.5 tabular-nums text-[var(--ink-muted)]">
      <span
        aria-hidden
        ...
        <thead>
          <tr className="bg-[var(--surface-2)] text-left text-xs uppercase tracking-wider text-[var(--ink-dim)]">
            ...
            <th className="hidden px-3 py-3 font-medium sm:table-cell">
              <abbr title="Modelled retirement (DNF) hazard" className="no-underline">
                DNF Risk
              </abbr>
            </th>
