...
import { totalLaps, trackTypeLabel } from "@/lib/track";
import { useReducedMotion } from "@/lib/useReducedMotion";
import type { TrackType } from "@/types/nascar";

interface HUDHeaderProps {
  round: number;
  /** Venue name shown as the headline (e.g. "Daytona International Speedway"). */
  name: string;
  /** Marketing race title (e.g. "DAYTONA 500"), when the export ships it. */
  raceName?: string | null;
  /** Country string for the flag + meta line. */
  country: string | null;
  /** Whether the round has run (drives the live/upcoming pill). */
  completed: boolean;
  /** NASCAR track archetype — surfaced as the format indicator. */
  trackType: TrackType;
  /** Advertised laps per stage (three-stage race format). */
...
/**
 * Telemetry-framed race header — the NASCAR analogue of the F1 flagship's
 * HUDHeader, adapted to Cup's single-race, three-stage weekends. The
 * right-hand strip carries the track-type badge (superspeedway / intermediate
 * / short track / road course) and the playoff marker on Chase rounds.
 */
export default function HUDHeader({
  ...
  const laps = totalLaps(stageLaps);
      ...
      <HUDPanel
        ...
        rightSlot={
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            {isPlayoff && <Badge variant="live">Playoffs</Badge>}
        ...
        <div className="-mx-5 grid grid-cols-1 gap-0 px-5 sm:-mx-6 sm:px-6 md:grid-cols-3">
          ...
          <div className="row-spec md:border-b-0 md:px-6">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {stageLaps && stageLaps.length > 0 && (
                <span className="font-mono text-[11px] uppercase tracking-[0.16em] text-[color:var(--muted)]">
                  Stages {stageLaps.join(" / ")}
                </span>
              )}
            </div>
            <p className="body-sm mt-2 text-[color:var(--muted)]">
              {laps
                ? `${stageLaps!.length} stages, ${laps} laps${isPlayoff ? " — a Chase playoff round" : ""}.`
                : isPlayoff
                  ? "A Chase playoff round."
