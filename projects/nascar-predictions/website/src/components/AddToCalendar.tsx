 ...
 * calendar file in the browser (no server, no network) and triggers a download.
 * Safe for the static export — all iCalendar work is done by the pure builders
 * in `@/lib/calendar` and the Blob/anchor dance only runs inside the click
 * handler.
 *
 * Pass a single `race` to download one event, or `races` (the full season
 * calendar) to download every round. Both come straight from nascar.json's
 * `calendar[*]` entries (mapped to the `{ round, name, circuit, date, country,
 * isPlayoff }` shape).
 */
    ...
    <Button
      ...
      aria-label={
        race
          ...
          : "Add the full NASCAR Cup season to your calendar"
      }
    >
      <CalendarIcon />
      {resolvedLabel}
