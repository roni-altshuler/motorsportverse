 ...
 * round, coloured by status. Completed rounds are solid (NASCAR yellow);
 * upcoming rounds are hollow; the ten Chase rounds carry a playoff tick under
 * the dot. The next round pulses + glows. Each dot links to the round's
 * detail page. The pulse honours prefers-reduced-motion.
 */
          ...
          {calendar.map((race, i) => {
            ...
            return (
              <motion.div
                ...
                title={`R${race.round}: ${race.name}${race.isPlayoff ? " (Playoffs)" : ""}`}
              >
                <Link
                  ...
                  style={{
                    ...
                    outline: race.isPlayoff
                      ? "1px solid color-mix(in srgb, var(--accent) 55%, transparent)"
                      : undefined,
                    outlineOffset: race.isPlayoff ? "2px" : undefined,
                    boxShadow: isNext ? "0 0 12px var(--accent)" : undefined,
                  }}
                  aria-label={`Round ${race.round}: ${race.name}${race.isPlayoff ? " (playoff round)" : ""}`}
                />
                {isNext && !reduced && (
                  <motion.span
                    aria-hidden
