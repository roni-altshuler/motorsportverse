...
Design — mirrors the FE port of ``f1_prediction_utils.train_ensemble``:
...
* **Features** (all leakage-safe, prior-rounds-only): rolling mean/median
  ...
  driver Elo, rookie flag, the NASCAR-specific **track-type split** (mean
  finish at the target round's track type minus overall — the specialist
  signal) and the rolling **DNF rate** (attrition drags mean finish and the
  learner should know why).
...
Era window: the learned signal trains only within the NextGen window
...
off, deps missing, pre-NextGen season, or too little data all return ``None``
...
FEATURE_COLUMNS: list[str] = [
    ...
    "roll_median_finish",   # median finishing position (robust to one big wreck)
...
def predict_ml_skill(
    ...
    *,
    ...
    when the ML path is disabled, the season predates the NextGen training
    window, the dependencies are missing, or there is too little data to learn
    anything. The caller (:func:`.model.estimate_skill`) treats ``None`` as
    "skip this signal", exactly like the other optional paths.
    """
    if (
