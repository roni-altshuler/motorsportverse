"""Season rollover — archive a completed NASCAR season and start the next one.

Ported from the FE/F3 template (itself from the F1 flagship) and adapted to
NASCAR's conventions: calendar-year seasons, the season lives in ``config.py``
(2026 literals + announced-calendar overrides), results come from the
committed snapshot ``data/official_<year>.json``, and the website data
contract is produced by ``nascar_predictions.export`` into
``website/public/data/``.
  ...
  --auto             If the active season is COMPLETE (the snapshot says every
                     ...
                     refreshes seasons.json — safe to call from the cron on
                     every run.

All steps support --dry-run.

Run:  python -m nascar_predictions.season_rollover --auto [--dry-run]
"""
    ...
    *,
    ...
    if year != config._DEFAULT_SEASON and _load_announced(year, announced_dir) is None:
        raise SystemExit(
            ...
            f"`python -m nascar_predictions.bootstrap_next_season --year {year}` first."
        )
    ...
    print(
        f"Season {year} is now active. Run `python -m nascar_predictions.refresh` and "
        "`python -m nascar_predictions.export` to populate it."
    )
    ...
    *,
    ...
    if announced is None:
        return _no_op(
            ...
            f"— run `python -m nascar_predictions.bootstrap_next_season`."
        )
