"""Snapshot NASCAR source — real results from the committed offline snapshots.

Reads ``data/official_<SEASON>.json`` (produced by
:mod:`nascar_predictions.refresh` from the cf.nascar.com feeds) for the active
season, and ``data/seasons/<year>.json`` (produced by
:mod:`nascar_predictions.backfill`) for archived seasons. Serves the **real**
classified order for completed rounds and returns ``None`` for rounds it has
no data for, so the composite falls through to the synthetic generator. This
makes the default pipeline real *and* offline/deterministic — no network at
build or test time.
"""
...
# snapshot.py lives in src/nascar_predictions/sources/, so the project root
...
class SnapshotNascarSource:
    ...
    def stage_results(self, year: int, round: int) -> dict[str, list[dict]] | None:
        """Per-stage top-10 rows captured in the snapshot, or None."""
        snap = self._snap(year)
        if not snap or year != snap.get("season"):
            return None
        block = snap.get("results", {}).get(str(round))
    ...
    def qualifying(self, year: int, round: int) -> list[str] | None:
        ...
        ``refresh`` stores the pole-qualifying classification under
        ``snapshot["qualifying"][str(round)]`` — including the *upcoming*
        round once its session has run but before the race, driving the
        post-quali forecast.
        """
        snap = self._snap(year)
        if not snap or year != snap.get("season"):
    ...
    def entry_list(self, year: int, round: int) -> list[str] | None:
        """Pre-race entry list for an upcoming round captured by refresh."""
        snap = self._snap(year)
        if not snap or year != snap.get("season"):
            return None
        entries = snap.get("entries", {}).get(str(round))
