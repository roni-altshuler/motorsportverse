"""Forward-time evaluation for NASCAR — scores leakage-safe predictions vs actuals.

**This is the headline validation surface for the Cup model.** For every
...
**full classified field**: NASCAR classifies every car (retirees keep a
finishing position), so — unlike the open-wheel projects' finishers-only
convention — every started car counts. The DNF-composed model is scored on
exactly what it forecasts.
...
* ``gridOrder`` — the round's actual starting grid as the prediction (a real
  pre-race-known baseline; over a 400-mile race it is beatable, which is
  exactly why it must be beaten, not ignored).
...
Run:  python -m nascar_predictions.forward_eval --season 2026 [--allow-empty]
...
def _actuals(source: NascarDataSource, year: int, rnd: int) -> dict[str, int]:
    return {
...
def _grid_map(source: NascarDataSource, year: int, rnd: int) -> dict[str, int] | None:
...
def _season_summary(year: int, rounds: list[dict]) -> dict:
    ...
    return {
        ...
        # NASCAR classifies retirees, so scoring covers the whole field.
        "finishersOnly": False,
...
def main() -> int:
    ...
    try:
        evidence_path = write_evidence_artifact(args.out, "nascar")
