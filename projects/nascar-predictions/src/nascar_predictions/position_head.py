"""Direct finishing-position head for NASCAR (opt-in alternative race-order head).

NASCAR port of the F1 flagship's ``models/position_model.py`` (A/B-gated,
verdict-driven; FE/F3 carry the same port). The production race order is a
*post-processing* of the latent-skill blend: :mod:`.model` sorts drivers by
simulated mean finish from a pace score + hazard — it never learns finishing
position directly. This module learns finishing position **directly**,
leakage-safe, so it can be A/B'd against the production path before anyone
trusts it.

Gate
----
The head is wired behind the ``NASCAR_USE_POSITION_HEAD`` environment flag
...
re-ranking on top of the production signal. Cup rounds carry a single race,
so the frame has one row per (round, classified car); the ``dnfProbability``
and track-type flags carry NASCAR's attrition and specialisation structure.
...
ENV_FLAG = "NASCAR_USE_POSITION_HEAD"
RANDOM_STATE = 42

# Cup rounds contribute ~38 rows each (everyone is classified), so two prior
# rounds already give ~75 rows; three keeps parity with the family default.
...
FEATURE_NAMES: tuple[str, ...] = (
    ...
    "isSuperspeedway",   # track-type one-hots (intermediate = baseline)
    "isShort",
    "isRoad",
)
...
MONOTONIC_CONSTRAINTS: tuple[int, ...] = (1, 1, -1, 1, 0, 1, 1, 0, 0, 0)
...
def head_enabled(override: bool | None = None) -> bool:
    """A/B gate: explicit override wins, else the ``NASCAR_USE_POSITION_HEAD``
...
def extract_race_features(
    race_fc: "RaceForecast", *, track_type: str = "intermediate"
    ...
    for i, c in enumerate(codes):
        ...
        out[c] = {
            ...
            "isSuperspeedway": 1.0 if track_type == "superspeedway" else 0.0,
            "isShort": 1.0 if track_type == "short" else 0.0,
            "isRoad": 1.0 if track_type == "road" else 0.0,
        }
    return out
...
def build_training_frame(
    forecasts_by_round: Mapping[int, "RoundForecastNascar"],
    ...
    ``actual_by_round`` maps round → ``{driver: position}`` (the full Cup
    classification — NASCAR classifies every car, retirees included).
    ...
    assert_prior_only(
        prior_map, current_round=int(target_round), label="nascar.position_head.training_rounds"
    )
...
class PositionHead:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "PositionHead":
        ...
        # the frame is small (a handful of rounds × ~38 rows).
        try:
...
# --------------------------------------------------------------------------- #
# Wiring against the NASCAR data/model layer
# --------------------------------------------------------------------------- #
def _actuals_for_round(source: "NascarDataSource", year: int, rnd: int) -> dict[str, int]:
    return {
...
def _prior_replays(
    source: "NascarDataSource", year: int, target_round: int
) -> tuple[dict[int, "RoundForecastNascar"], dict[int, dict[str, int]]]:
    ...
    *production* signal — never the head's own output (no feedback loop).
    ...
    forecasts: dict[int, "RoundForecastNascar"] = {}
...
def train_for_round(
    ...
    *,
    ...
    forecasts_by_round: Mapping[int, "RoundForecastNascar"] | None = None,
