...
from nascar_predictions import config, export, pipeline
...
def test_gate_open_on_real_snapshot(real_source):
    ...
    # Track-type strata: 19 completed rounds cover all four types.
    strata = calibrator.strata_with_models()
    assert "superspeedway" in strata or "intermediate" in strata
...
def test_calibrated_payload_keeps_raw_alongside(real_source):
    calibrator, real_rounds = export.build_calibrator(real_source, SEASON)
    fc = pipeline.forecast_round(real_source, SEASON, 20)
