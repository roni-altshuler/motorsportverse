"""The unique-F2-model behaviours: sprint vs feature, reverse grid, rookies."""

import pytest

from f2_predictions import config, model
...
def test_forecast_shapes_and_ranges(forecast):
    for race in (forecast.sprint, forecast.feature):
        assert len(race.order) == 22
        assert sorted(race.order) == sorted(d["code"] for d in config.DRIVERS)
        for code in race.order:
            assert 1 <= race.range_low[code] <= race.range_high[code] <= 22
            assert 1.0 <= race.mean_finish[code] <= 22.0
...
def test_championship_scales_with_remaining_rounds(source):
    ...
    few = model.project_championship_f2(points, skill, remaining_rounds=1, n_samples=800)
    many = model.project_championship_f2(points, skill, remaining_rounds=5, n_samples=800)
