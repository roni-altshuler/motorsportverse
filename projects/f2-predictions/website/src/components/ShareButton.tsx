 ...
 *
 * It only needs to hand over the current page URL (read lazily from
 * `window.location` so it stays correct under static export / base paths).
 */
import { useCallback, useState, useSyncExternalStore } from "react";

interface ShareButtonProps {
  /** Headline used by the native sheet + tweet text. */
  title: string;
...
export default function ShareButton({ title, text, className }: ShareButtonProps) {
  ...
  const shareText = text ?? `${title} — F2 race prediction & win-probability model`;
