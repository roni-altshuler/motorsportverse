 ...
 * classified result for one F2 driver. Every F2 round scores two races, so a
 * round contributes up to two rows (Sprint then Feature), each tagged with a
 * race-type badge. Only races with an official result are shown; each row links
 * through to the full race page.
 *
 * Predicted position comes from each race block's `classification` order; the
 * actual position + base points are distilled upstream into `DriverRaceResult`.
 * There is no finish-status / DNF field in the F2 export, so (unlike the F1
 * flagship's table) no status column is shown rather than one being faked.
 */
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="text-left">
            {["Round", "Race", "Grand Prix", "Pred.", "Actual", "Δ", "Pts"].map(
              (h, i) => (
                <th
                  key={h}
          ...
          {rows.map((r) => {
            ...
            return (
              ...
              >
                ...
                <td className="py-2.5">
                  <Badge variant={r.raceType === "feature" ? "info" : "muted"}>
