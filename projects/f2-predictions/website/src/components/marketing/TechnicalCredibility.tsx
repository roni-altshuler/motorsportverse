 ...
 * F2: reliability, freshness, reproducibility, openness, and the shared
 * MotorsportVerse core. Freshness comes in as a build-time prop; F2 has no
  ...
  const pillars: Pillar[] = [
    {
      ...
      body: "An automated pipeline folds in the latest sprint and feature result before the next round — recent form and grid position, no manual intervention.",
    },
    {
      Icon: Clock,
    ...
    },
    {
      ...
      body: roundsGraded
        ? `Built on the same motorsport-core that powers RaceIQ F1, with ${roundsGraded} F2 ${roundsGraded === 1 ? "round" : "rounds"} graded so far this season.`
        : "Built on the same motorsport-core that powers RaceIQ F1 — one engine, tuned for a spec series where driver skill rules.",
    },
  ];
