/**
 * Race photography mapping — one curated aerial circuit photo per F2 round.
 *
 * F2 shares the F1 race weekends and circuits, so this is a direct port of the
 * RaceIQ F1 raceArt map (src/lib/raceArt.ts), re-keyed to the F2 calendar's
 * venue `key` strings (as emitted by the F2 export: sakhir, jeddah, melbourne,
