/**
 * calendar.ts — pure, dependency-free iCalendar (.ics) builders so a fan can add
 * an F2 round — or the whole season — to Apple Calendar, Google Calendar,
 * Outlook, etc. Ported from the RaceIQ F1 flagship and re-themed for F2.
 *
 * Everything here is a pure function of its inputs (plus an injectable `now` for
 * deterministic tests), so it is safe to import from both server and
 * `"use client"` code in a static export.
 *
 * F2's calendar entries carry a race *date* (the feature-race day) but no
 * confirmed lights-out *time*, so by default each round is written as an all-day
 * VEVENT on race day (DTSTART;VALUE=DATE). When a real start time is known it can
 * be passed via `startUtc` and a timed, 2-hour UTC event is emitted instead — no
...
/** Minimal shape of a round, matching f2.json `calendar[*]` entries. */
export interface CalendarRace {
  round: number;
  /** e.g. "Australia" (the F2 export uses the country as the round name). */
  name: string;
  /** e.g. "Melbourne" — the host city / circuit locale. */
  circuit?: string;
  /** ISO date, "YYYY-MM-DD" (the feature-race day). */
  date: string;
  /** e.g. "Australia" */
  country?: string;
  laps?: number;
...
const PRODID = "-//RaceIQ//F2 Race Calendar//EN";
...
function stableUid(race: CalendarRace, season?: number): string {
  ...
  return `f2-${yr}-round-${race.round}@raceiq`;
}

/** Build the VEVENT block (array of unfolded content lines) for one round. */
function buildVeventLines(race: CalendarRace, opts: IcsOptions): string[] {
  ...
  const descParts = [
    `Round ${race.round} of the${opts.season ? " " + opts.season : ""} FIA Formula 2 season.`,
    "Reversed-grid sprint + feature race.",
  ];
  if (race.circuit) descParts.push(`Host: ${race.circuit}.`);
  descParts.push("Predictions & standings at RaceIQ F2.");
  ...
  lines.push(`SUMMARY:${escapeText(`${race.name} — F2`)}`);
...
/** Build a complete .ics string containing a single round event. */
export function buildRaceIcs(race: CalendarRace, opts: IcsOptions = {}): string {
  const name = `${race.name}${opts.season ? ` (F2 ${opts.season})` : " (F2)"}`;
...
export function buildSeasonIcs(
  ...
  const name = `Formula 2${opts.season ? ` ${opts.season}` : ""} Season — RaceIQ`;
...
export function icsFilename(race?: CalendarRace, season?: number): string {
  if (!race) return `f2-${season ?? "season"}-calendar.ics`;
  const slug = race.name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/(^-|-$)/g, "");
  return `${slug || `round-${race.round}`}-f2${season ? `-${season}` : ""}.ics`;
}
