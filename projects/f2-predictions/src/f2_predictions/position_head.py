"""Direct finishing-position head for F2 (opt-in alternative race-order head).

This is the F2 port of the F1 flagship's ``models/position_model.py``
(landed in commit 189db5b, A/B-gated, verdict-driven). The production F2 race
...
Gate
----
The head is wired behind the ``F2_USE_POSITION_HEAD`` environment flag
...
FastF1 feature matrices is slow and rate-limited. F2 has no such constraint:
...
production classification per round; F2's production model routes *two*
...
14-round season. The ``gridPosition`` + ``isSprint`` features carry the
...
ENV_FLAG = "F2_USE_POSITION_HEAD"
RANDOM_STATE = 42

# F2 has a 14-round season and ~44 finisher-rows per weekend (22-car grid × two
...
def head_enabled(override: bool | None = None) -> bool:
    """A/B gate: explicit override wins, else the ``F2_USE_POSITION_HEAD`` env
...
def build_training_frame(
    forecasts_by_round: Mapping[int, "RoundForecastF2"],
    ...
    assert_prior_only(prior_map, current_round=int(target_round), label="f2.position_head.training_rounds")
...
class PositionHead:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "PositionHead":
        ...
        # the frame is small (a handful of rounds × ~44 rows).
        try:
...
# --------------------------------------------------------------------------- #
# Wiring against the F2 data/model layer
# --------------------------------------------------------------------------- #
def _actuals_for_round(source: "F2DataSource", year: int, rnd: int) -> dict[str, dict[str, int]]:
...
def _prior_replays(
    source: "F2DataSource", year: int, target_round: int
) -> tuple[dict[int, "RoundForecastF2"], dict[int, dict[str, dict[str, int]]]]:
    ...
    *production* signal — never the head's own output (no feedback loop).
    ...
    forecasts: dict[int, "RoundForecastF2"] = {}
...
def train_for_round(
    ...
    *,
    ...
    forecasts_by_round: Mapping[int, "RoundForecastF2"] | None = None,
...
def maybe_rerank_round(
    ...
    *,
    ...
    graceful degradation contract as the other optional F2 signals.
    """
