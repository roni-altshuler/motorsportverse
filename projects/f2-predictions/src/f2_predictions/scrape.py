"""Proof CLI: scrape a real F2 season and compute real standings via the core.

Demonstrates the Phase 3 real feed end-to-end — fiaformula2.com → ``FiaF2Source``
→ ``motorsport_core.standings`` — for a completed season, so the output can be
checked against the known champion (2024: Bortoleto / Invicta Racing).
...
Run:  python -m f2_predictions.scrape --season 2024
"""
...
def scrape_standings(year: int):
    ...
    if not n:
        raise SystemExit(
            f"no anchor raceid for {year} — add it to config.FIA_F2_SEASON_ANCHORS"
        )
...
def main() -> int:
    ...
    print(f"FIA F2 {args.season} — scraped {n} rounds (sprint + feature)\n")
