...
def predict_ml_skill(
    ...
    if os.getenv("F2_USE_TEMPORAL_SKILL", "0") == "1":
...
def predict_temporal_candidate(source, year, prior_rounds, field_mean):
    ...
    try:
        ...
        real_rounds = [r for r in prior_rounds if all(CompositeF2Source.is_real(source.provenance(year, r, race_index=i)) for i in (0, 1))]
