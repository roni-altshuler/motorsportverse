"""Promotion gate for F2 — thin CLI over motorsport_core.promotion.
...
F1-parity port of commit 189db5b, A/B-gated by ``F2_USE_POSITION_HEAD`` and OFF
by default). Its walk-forward evidence is the ``forward_eval/position_model_ab.json``
artifact written by ``python -m f2_predictions.forward_eval --position-model-ab``:
...
Run:  python -m f2_predictions.promotion_decision --season 2026 [--allow-empty]
"""
...
def build_status(data_dir: Path) -> dict:
    ...
    return {
        ...
        "candidateFlag": "F2_USE_POSITION_HEAD",
