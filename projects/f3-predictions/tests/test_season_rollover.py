...
from f3_predictions import bootstrap_next_season, config, export, season_rollover
...
def site_dir(tmp_path: Path) -> Path:
    ...
    data.mkdir()
    (data / "f3.json").write_text(json.dumps({"season": 2026}))
...
def _write_snapshot(snapshot_dir: Path, year: int, completed: int, total: int = 9) -> None:
...
def test_season_complete_from_snapshot(snapshot_dir):
    _write_snapshot(snapshot_dir, 2026, completed=9, total=9)
    assert season_rollover.season_complete(2026, snapshot_dir=snapshot_dir)
    _write_snapshot(snapshot_dir, 2026, completed=5, total=9)
...
def test_auto_noop_until_next_season_begins(site_dir, snapshot_dir):
    _write_snapshot(snapshot_dir, config.SEASON, completed=9)
    announced = snapshot_dir / "announced_seasons"
...
def test_auto_rolls_over_once_complete_and_begun(site_dir, snapshot_dir):
    ...
    _write_snapshot(snapshot_dir, year, completed=9)
    announced = snapshot_dir / "announced_seasons"
    ...
    assert (site_dir / "seasons" / str(year) / "f3.json").exists()
