...
The parser is exercised against a saved fixture (a trimmed real fiaformula3.com
results page); nothing here touches the network, so it is CI-safe. The live
end-to-end path is the manual `f3_predictions.scrape` CLI.
"""

import builtins
from pathlib import Path

from f3_predictions.sources.fia_f3_source import FiaF3Source

FIXTURE = (Path(__file__).parent / "fixtures" / "fia_f3_round.html").read_text()


def test_parses_feature_classification():
    rows = FiaF3Source._parse_session(FIXTURE, "Feature Race")
...
def test_parses_sprint_and_differs_from_feature():
    feature = FiaF3Source._parse_session(FIXTURE, "Feature Race")
    sprint = FiaF3Source._parse_session(FIXTURE, "Sprint Race")
...
def test_missing_session_returns_empty():
    assert FiaF3Source._parse_session("<html><body>nope</body></html>", "Feature Race") == []
