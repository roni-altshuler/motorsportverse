...
from f3_predictions import backfill, config
...
def test_backfill_writes_both_races(tmp_path):
    ...
    # 2 races × roster drivers × completed rounds (a row per roster driver, whether
    # or not they were classified — DNFs get a null actual_position).
    assert written == 2 * len(config.DRIVERS) * config.COMPLETED_ROUNDS
    ...
    try:
        ...
        assert len(pairs) > 0
        assert all(1 <= p <= len(config.DRIVERS) and 1 <= a <= len(config.DRIVERS) for p, a in pairs)
    finally:
        store.close()
