"""Regression tests for the wrong-event ingestion guards (F3).
...
F3 scrapes fiaformula3.com per-raceid instead, but the same class of bug —
...
that make that impossible:
  ...
  * ``FiaF3Source.results``/``qualifying`` refuse (return ``None`` → the
...
import pytest
from f3_predictions import config, refresh
from f3_predictions.sources.fia_f3_source import FiaF3Source, WrongEventError

SEASON = config.SEASON


def _round_page(round_num: int, country: str, city: str, codes: list[str]) -> str:
    """A minimal fiaformula3.com-shaped round page: title + Feature/Sprint tables."""
    ...
    title = (
        f"Formula 3 {SEASON} Result for Round {round_num} : {country} , "
...
_CODES = ["AAA", "BBB", "CCC"]
SILVERSTONE = _round_page(5, "United Kingdom", "Silverstone", _CODES)
AUSTRIA = _round_page(4, "Austria", "Spielberg", _CODES)
# A results table with NO round title — the shape of a truncated/garbage page.
GARBAGE = SILVERSTONE.split("</h1>", 1)[1]


def _offline_source(round_num: int, page: str) -> FiaF3Source:
    """A FiaF3Source whose caches are primed so no network happens: the round
...
def test_identity_accepts_the_requested_round():
    identity = FiaF3Source.verify_page_identity(SILVERSTONE, expected_round=5)
    assert identity["round"] == 5
    assert identity["country"] == "United Kingdom"


def test_identity_rejects_another_rounds_page():
    with pytest.raises(WrongEventError, match="round 4"):
        FiaF3Source.verify_page_identity(AUSTRIA, expected_round=5)
...
def test_identity_rejects_country_mismatch():
    ...
    with pytest.raises(WrongEventError, match="Austria"):
        FiaF3Source.verify_page_identity(
            _round_page(5, "Austria", "Spielberg", _CODES),
            expected_round=5,
...
def test_identity_accepts_matching_country_and_parenthetical_variants():
    FiaF3Source.verify_page_identity(
        SILVERSTONE, expected_round=5, expected_country="United Kingdom"
    )
    # Round 9: config says "Spain" for Madrid; a "Spain (Madrid)" label folds to it.
    madrid = _round_page(9, "Spain", "Madrid", _CODES)
    FiaF3Source.verify_page_identity(
        madrid, expected_round=9, expected_country="Spain (Madrid)"
    )
...
def test_identity_rejects_truncated_or_garbage_page():
    for page in (GARBAGE, "", None, "<html>503 Service Unavailable</html>"):
        with pytest.raises(WrongEventError, match="no parseable round title"):
            FiaF3Source.verify_page_identity(page, expected_round=5)


# ── results()/qualifying(): the fetch path refuses, the composite defers ────

def test_results_refuse_wrong_event_page():
    src = _offline_source(5, AUSTRIA)  # raceid resolves, but serves round 4's page
    assert src.results(SEASON, 5, race_index=1) is None
    assert src.results(SEASON, 5, race_index=0) is None
...
def test_results_refuse_garbage_page_despite_parseable_table():
    ...
    src = _offline_source(5, GARBAGE)
    assert src.results(SEASON, 5, race_index=1) is None


def test_results_accept_the_correct_event():
    src = _offline_source(5, SILVERSTONE)
    rows = src.results(SEASON, 5, race_index=1)
...
def _patch_refresh_feed(monkeypatch, page: str, round_num: int = 1):
    ...
    monkeypatch.setattr(FiaF3Source, "calendar", lambda self, year: [entry])
    monkeypatch.setattr(FiaF3Source, "_page", lambda self, raceid: page)
...
def test_refresh_raises_on_wrong_event_and_writes_no_snapshot(monkeypatch, tmp_path):
    ...
    # Round 1 (Australia)'s raceid serves the Austria round-4 page.
...
def test_committed_snapshot_calendar_matches_config_countries():
    ...
    Venue.country, matches the FIA page titles for all 9 rounds)."""
    snap_path = refresh._DEFAULT_OUT
    if not snap_path.exists():
        pytest.skip("no committed snapshot")
    snap = json.loads(snap_path.read_text())
    fold = FiaF3Source._normalise_country
