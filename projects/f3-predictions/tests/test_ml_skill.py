...
from f3_predictions import config, ml_skill, model
...
def test_estimate_skill_with_ml_is_a_valid_pace_mapping(source):
    ...
    assert all(80.0 < v < 100.0 for v in pace.values())  # same band the model_f3 tests assert
...
def test_temporal_candidate_holds_out_events_and_is_opt_in(source, monkeypatch):
    ...
    monkeypatch.setenv("F3_USE_TEMPORAL_SKILL", "1")
