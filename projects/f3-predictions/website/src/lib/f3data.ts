// Build-time loaders for the F3 dataset (generated by f3_predictions.export and
...
let cache: F3Data | null = null;

export function getF3Data(): F3Data {
  if (cache) return cache;
  cache = readJson<F3Data>("f3.json");
  if (!cache) throw new Error("f3.json missing — run `python -m f3_predictions.export` first");
  return cache;
}
...
export function allRoundNumbers(): number[] {
  return getF3Data().calendar.map((c) => c.round);
}
...
// completed round's sprint + feature finishing order using the F3 points table,
