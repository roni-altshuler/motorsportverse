 ...
 * F3's real data contract (`@/types/f3`): one season file (`f3.json`) whose
 * `driverStandings[*]` carry the identity + cumulative `pointsHistory`, and per
 * round `rounds/round_NN.json` files that each hold TWO scored races — a
 * reversed-grid `sprint` and a merit `feature`. Every driver result is
 * therefore per-race, not per-round.
 *
...
// The official F3 points tables. These are the same tables the site already
// uses to reconstruct the standings progression (see lib/f3data.ts) — kept in
// sync here so the per-race "points" column is consistent across the site.
// Base points only (pole / fastest-lap bonuses are not attributable per race).
const F3_FEATURE_POINTS = [25, 18, 15, 12, 10, 8, 6, 4, 2, 1];
const F3_SPRINT_POINTS = [10, 8, 6, 5, 4, 3, 2, 1];
...
export interface DriverRaceResult {
  ...
  /** Base championship points for the finish (from the F3 table), else null. */
  points: number | null;
...
function raceResult(
  ...
  const table = block.raceType === "sprint" ? F3_SPRINT_POINTS : F3_FEATURE_POINTS;
...
/** Count of top-10 (points-paying feature) or top-8 (sprint) finishes. */
...
export function allDriverCodes(
  data: Pick<F3Data, "driverStandings"> | null,
): string[] {
