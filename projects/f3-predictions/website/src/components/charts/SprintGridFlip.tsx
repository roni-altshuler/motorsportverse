// Sprint grid-flip explainer — F3's answer to the F1 flagship's
// StrategyExplorer. F1 explores tyre-stop strategies (it has a race
// simulator); F3 has no telemetry or strategy data, but it has something F1
...
// pack and have to carve forward. That flip IS the strategic story of an F3
...
import type { ClassificationEntry, GridEntry } from "@/types/f3";
 ...
 *  top-N reversed (F3 2026 uses 12; detected, not assumed). */
...
export function SprintGridFlip({
  ...
  const shown = flipSize > 0 ? sprintGrid.slice(0, flipSize) : sprintGrid.slice(0, 12);
