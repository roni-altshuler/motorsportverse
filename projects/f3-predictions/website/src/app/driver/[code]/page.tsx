...
export async function generateMetadata({
  ...
  const title = `${fullName} — F3 ${seasonYear} Driver Profile`;
  const bits: string[] = [];
  ...
  const description =
    bits.length > 0
      ? `${fullName}: ${bits.join(", ")}. Season form, points progression, and predicted-vs-actual results for the ${seasonYear} FIA Formula 3 season.`
      : `${fullName}'s ${seasonYear} FIA Formula 3 season profile: form, points progression, and predicted-vs-actual results.`;
