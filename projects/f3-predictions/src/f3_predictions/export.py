"""Generate the F3 website's JSON data from the model + pipeline.
...
flagship's fan-out shape so the two sites can share components 1:1:
    ...
    public/data/
      f3.json                     season summary (calendar, standings, championship,
...
Run:  python -m f3_predictions.export   [--out <dir>]
"""
...
def _actual_map(source: F3DataSource, year: int, rnd: int, race_type: str) -> dict[str, int]:
    results = source.race_results_for_round(year, rnd)[race_type]
    return {r.competitor: r.position for r in results}


def _next_round(source: F3DataSource, year: int) -> int | None:
...
def _race_block(
    race: RaceForecast, source: F3DataSource, year: int, rnd: int, completed: bool
) -> dict:
...
def probabilities_payload(fc: RoundForecastF3, calibrator, real_rounds: int) -> dict:
...
def build_calibrator(source: F3DataSource, year: int):
    ...
    real_rounds = [
        ...
        if CompositeF3Source.is_real(source.provenance(year, r, race_index=1))
    ]
...
def _championship(source: F3DataSource, year: int) -> list[dict]:
...
# --------------------------------------------------------------------------- #
def _season_accuracy(round_forecasts: dict[int, RoundForecastF3], source: F3DataSource, year: int) -> dict:
    scored = 0
...
def _standings_lists(source: F3DataSource, year: int) -> tuple[list[dict], list[dict]]:
    """(driverStandings, teamStandings) for f3.json — official snapshot totals
...
def build_payload(
    round_forecasts: dict[int, RoundForecastF3],
    source: F3DataSource,
    year: int,
...
def _calibration_summary(calibrator, real_rounds: int) -> dict:
    ...
    return {
        ...
        "dataLimitation": (
            ...
            if applied
            else "F3 runs on a synthetic source by default; probability calibration turns on "
            f"once {config.MIN_REAL_ROUNDS_FOR_CALIBRATION} real rounds are backfilled "
            "(set F3_USE_LIVE_RESULTS=1 with a live feed)."
        ),
