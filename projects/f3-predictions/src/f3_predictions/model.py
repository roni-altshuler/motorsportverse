"""The unique F3 prediction model.

F3 is not F1 and it is not the thin pace-average predictor this project shipped
...
2. **Two structurally different races per weekend.** The **feature race** starts
   ...
   **top-10 reversed**. A fast driver lining up P10 in the sprint has to overtake
   to win — that reverse grid is the single most F3-specific thing here and has no
   F1 analogue.
...
and race-type logic is the genuinely new, F3-specific code — and it stays in the
...
def _prior_history(
    source: F3DataSource, year: int, prior_rounds: list[int]
...
# --------------------------------------------------------------------------- #
def _elo_skill(source: F3DataSource, year: int, current_round: int) -> tuple[
...
def estimate_skill(
    source: F3DataSource, year: int, current_round: int
) -> dict[str, float]:
    ...
    leakage.assert_prior_only(
        {r: None for r in prior_rounds}, current_round=current_round, label="f3.model.skill"
    )
...
def rookie_flags(source: F3DataSource, year: int, current_round: int) -> dict[str, bool]:
...
def _reverse_grid(merit_order: list[str]) -> list[str]:
    """F3 sprint grid: reverse the top ``REVERSE_GRID_SIZE`` of the merit order."""
...
def project_championship_f3(
    ...
    *,
    ...
    single points table per race; F3 awards a different table to the sprint and
