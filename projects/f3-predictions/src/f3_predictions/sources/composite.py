"""Composite F3 source — real feeds first, synthetic always last.
...
class CompositeF3Source:
    name = "composite"

    def __init__(self, sources):
        """``sources`` is an ordered list tried first→last. The last entry should
        be a :class:`SyntheticF3Source` so there is always a fallback."""
    ...
    def default():
        ...
        needs no network."""
        from .snapshot import SnapshotF3Source

        return CompositeF3Source([SnapshotF3Source(), SyntheticF3Source()])

    @staticmethod
    def live():
        """Live network feed first (fresh FIA scrape), then the committed
        snapshot, then synthetic. Used when ``F3_USE_LIVE_RESULTS=1`` — mainly to
        refresh data; builds normally use :meth:`default` for reproducibility."""
        from .fia_f3_source import FiaF3Source
        from .snapshot import SnapshotF3Source

        return CompositeF3Source([FiaF3Source(), SnapshotF3Source(), SyntheticF3Source()])
