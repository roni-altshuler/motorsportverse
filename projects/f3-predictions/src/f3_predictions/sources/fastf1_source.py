"""FastF1-backed F3 results — best-effort, probed at runtime.

FastF1's public API targets Formula 1; its Formula 3 coverage is partial and
version-dependent and there is no stable documented way to load an F3 session's
...
mapping below is written so that if/when FastF1 exposes F3 results, only this file
changes.
"""
...
# FastF1 session codes for the two F3 races on a weekend.
_SESSION_CODE = {0: "Sprint", 1: "Feature"}


class FastF1F3Source:
    name = SOURCE_NAME
    ...
    def _load(self, year: int, round: int, race_index: int) -> list[Result] | None:
        """Attempt to load an F3 classification via FastF1.

        FastF1 does not currently expose F3 sessions, so this returns ``None``.
        Kept as the single integration point: when F3 session loading lands, fill
