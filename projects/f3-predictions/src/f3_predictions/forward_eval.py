"""Forward-time evaluation for F3 — scores leakage-safe predictions vs actuals.

**This is the headline validation surface for the F3 model** (F1-parity, commit
...
the shared :func:`motorsport_core.eval.score_round` bundle, so F3's accuracy
...
Run:  python -m f3_predictions.forward_eval --season 2026 [--allow-empty]
...
def _actuals(source: F3DataSource, year: int, rnd: int) -> dict[str, dict[str, int]]:
...
def build_walk_forward_summary(rounds: list[dict]) -> dict:
    ...
    surface (F1's ``forward_eval/summary.json`` shape, embedded in F3's
    ``season.json``).
    """
    out: dict[str, dict] = {}
    for rt in RACE_TYPES:
