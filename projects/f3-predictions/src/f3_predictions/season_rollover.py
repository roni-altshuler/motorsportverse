"""Season rollover — archive a completed F3 season and start the next one.

Ported from the F1 flagship's ``scripts/season_rollover.py`` and adapted to
F3's package conventions: the season lives in ``config.py`` (2026 literals +
...
``f3_predictions.export`` into ``website/public/data/``.
  ...
  --auto             If the active season is COMPLETE (the snapshot says every
                     ...
                     refreshes seasons.json — safe to call from the F3 cron on
                     every run.

All steps support --dry-run.

Run:  python -m f3_predictions.season_rollover --auto [--dry-run]
"""
    ...
    *,
    ...
    if year != config._DEFAULT_SEASON and _load_announced(year, announced_dir) is None:
        raise SystemExit(
            ...
            f"`python -m f3_predictions.bootstrap_next_season --year {year}` first."
        )
    ...
    print(
        f"Season {year} is now active. Run `python -m f3_predictions.refresh` (once a "
        "feed exists) and `python -m f3_predictions.export` to populate it."
    )
    ...
    *,
    ...
    if announced is None:
        return _no_op(
            ...
            f"— run `python -m f3_predictions.bootstrap_next_season`."
        )
