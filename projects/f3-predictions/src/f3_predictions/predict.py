"""F3 predictor — implements the shared ``motorsport_core.interfaces.Predictor``.

Thin adapter: it conforms F3 to the ecosystem contract and delegates the actual
work to :mod:`f3_predictions.pipeline` (which is itself mostly calls into
motorsport-core). This is what lets the F3 project drop into any core-driven
...
class F3Predictor(Predictor):
    def __init__(self) -> None:
        self._fitted_upto: int | None = None

    def fit(self, source: F3DataSource, season: int, upto_round: int) -> None:
    ...
    def predict(self, source: F3DataSource, season: int, round: int) -> RoundForecast:
