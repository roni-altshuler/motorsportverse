"""Refresh the committed real-data snapshot from fiaformula3.com.

This is the *only* network-dependent step in the F3 pipeline. It scrapes the
...
Captured per season:
  * calendar   — 9 rounds (keys/names from config), with completed flags;
...
Run:  PYTHONPATH=src python -m f3_predictions.refresh [--season 2026] [--out data/official_2026.json]
"""
...
def _fetch(url: str, timeout: int = 25) -> str | None:
    ...
    try:
        resp = requests.get(
            url, headers={"User-Agent": "Mozilla/5.0 (motorsportverse/f3 research)"}, timeout=timeout
        )
...
def _score_pairs(row: str) -> list[tuple[float | None, float | None]]:
    ...
    The cells arrive as ``['', s1, f1, '', s2, f3, ...]`` (a separator per
...
def _parse_team_standings(page: str) -> list[dict]:
    ...
    for tr in re.split(r"<tr\b", body)[1:]:
        ...
        out.append(
            {
                "position": int(pos.group(1)),
                # Standings pages abbreviate some entrant names ("Hitech" for
                # "Hitech TGR") — normalise so they match config.TEAMS/TEAM_OF.
...
def build_snapshot(season: int) -> dict:
    ...
    for rnd in range(1, len(config.CALENDAR) + 1):
        ...
        sprint = FiaF3Source._parse_session(page, "Sprint Race", include_unclassified=True)
        feature = FiaF3Source._parse_session(page, "Feature Race", include_unclassified=True)
    ...
    driver_page = _fetch(f"{config.FIA_F3_BASE_URL}/Standings/Driver")
    team_page = _fetch(f"{config.FIA_F3_BASE_URL}/Standings/Team")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Refresh the F3 real-data snapshot from fiaformula3.com")
