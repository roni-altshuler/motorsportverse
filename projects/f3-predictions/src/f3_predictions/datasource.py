"""F3 data source — calendar/roster from config, results from a selectable feed.
...
- default (``F3_USE_LIVE_RESULTS`` unset/``"0"``): the **real** committed
  snapshot (``data/official_2026.json``, scraped from fiaformula3.com by
...
- live (``F3_USE_LIVE_RESULTS="1"``): a :class:`CompositeF3Source` that scrapes
...
a source directly via ``F3DataSource(source=...)``.
"""
...
def _live_enabled() -> bool:
    return os.getenv("F3_USE_LIVE_RESULTS", "0") == "1"


class F3DataSource(DataSource):
    sport = config.SPORT
    ...
    def __init__(self, *, live: bool | None = None, source=None):
        if source is not None:
            ...
            self._source = CompositeF3Source.live()
        else:
            self._source = CompositeF3Source.default()
    ...
    def completed_rounds(self, year: int = config.SEASON) -> list[int]:
        ...
        returns a non-empty classification. F3 rounds complete in order, so we
