"""F3 race-weekend orchestration — the cheap freshness gate the cron polls.

This is the F3 counterpart of the F1 flagship's ``gp_weekend.py``. The race-weekend
...
2. **Is there genuinely new data to publish?** (``check_work_pending``) — a single
   live probe of fiaformula3.com compared against the committed snapshot: fresh
...
Run (used by the workflow):
    python -m f3_predictions.race_weekend --detect-round
    python -m f3_predictions.race_weekend --round N --is-race-weekend
    python -m f3_predictions.race_weekend --round N --check-work-pending
    python -m f3_predictions.race_weekend --round N --phase
"""
...
# How wide a window around the weekend counts as "race weekend" for polling. F3
...
def _live_source():
    """A fresh fiaformula3.com probe (no snapshot/synthetic fallback).
...
def stranded_rounds(year: int = config.SEASON, *, live=None, now: datetime | None = None) -> list[int]:
    ...
    The F3 counterpart of F1's stranded-results backfill: a round is stranded when
    ...
    round whose result the (flaky, often-lagging) fiaformula3.com feed did not yet
...
def main() -> int:
    ap = argparse.ArgumentParser(description="F3 race-weekend freshness gate / phase detector")
