"""Per-lap race-pace ensemble for F3 — a ready seam, NOT active today.

Status: DORMANT / DATA-GATED. F3 lap-by-lap telemetry is not available from any
source this project wires up (the FIA F3 site publishes classifications, not
per-lap timing, and FastF1's F3 lap coverage is partial-to-absent). So this
...
Plackett-Luce ordering in :func:`f3_predictions.model.forecast_round`.
...
race model. F3's reverse-grid sprint would be a first-class win for that model:
...
target), flip :data:`f3_predictions.config.USE_RACE_SIMULATOR` on, and the rest
of the ensemble code below trains/persists without further plumbing.

Run:  python -m f3_predictions.train_race_pace --season 2026   # no-ops today
"""
...
LAP_FEATURE_COLUMNS: list[str] = [
    ...
    "track_temp",
    "rain_intensity",
    "is_sprint",  # F3-specific: sprint laps behave differently from feature laps
]
...
def lap_data_available(source: "F3DataSource", year: int) -> bool:
    """Whether a per-lap F3 telemetry feed is wired up. ``False`` today.
...
def load_lap_dataset(source: "F3DataSource", year: int, rounds: list[int]):
    ...
    raise NotImplementedError(
        "F3 lap-by-lap telemetry is not available. Implement this to return a "
...
def train_race_pace(source: "F3DataSource", year: int, rounds: list[int]) -> RacePaceEnsemble | None:
    ...
    :func:`f3_predictions.ml_skill.predict_ml_skill` / the F1 race-pace trainer:
...
def main() -> int:
    ...
    if ensemble is None:
        print(
            "train_race_pace: no F3 lap-by-lap data available — race simulator "
