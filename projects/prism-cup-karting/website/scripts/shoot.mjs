// Screenshot harness for the Prism Cup Karting website (MotorsportVerse).
...
//
// Default output: /tmp/prism-shots (kept out of the repo). Mirrors the
// ecosystem's per-site scripts/shoot.mjs pattern.
...
const SHOTS = process.argv[2] || "/tmp/prism-shots";
const PORT = 4342;
...
for (const [vp, width, height] of [
  ...
  for (const [name, route] of ROUTES) {
    ...
    // Scroll through the page so lazy content settles, then return to the
    // top before capturing.
    ...
    await page.mouse.wheel(0, -pageHeight - 2000);
