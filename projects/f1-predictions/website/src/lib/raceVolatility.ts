/**
 * Per-race volatility scoring.
 *
 * Each driver in a round's classification carries a finish-range
 * (`finishRangeLow`, `finishRangeHigh`) representing how wide the
 * model's per-driver position interval is. Drivers with tight
 * intervals signal a high-confidence forecast; wide intervals across
 * the front of the grid signal a chaotic race outlook (rain, street
 * circuit, fresh tyre rules, etc.).
 *
 * We collapse those per-driver widths into one scalar in [0, 1] by
 * averaging the top-6 widths and normalising against a reference
 * width chosen to match the bucketing thresholds below:
 *
 *   - score < 0.33  → "high confidence"    (tight front-end)
 *   - 0.33..0.66    → "moderate volatility"
 *   - score > 0.66  → "chaotic race"
 *
 * Tuning the buckets: the reference width (REF_WIDTH below) is
 * deliberately tight (4 positions). A driver predicted P3 with a
 * range of P2..P6 already pushes the score into the "moderate"
 * bucket; a P1 prediction with P1..P3 range stays comfortably high
 * confidence. Adjust REF_WIDTH if the buckets feel mistuned across
 * the season.
 */
...
/**
 * Compute a per-race volatility scalar from the top-N drivers'
 * prediction intervals. Returns `null` when the classification is
 * empty or has no range data populated.
 */
