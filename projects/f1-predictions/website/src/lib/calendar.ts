 ...
 * an F1 race — or the whole season — to Apple Calendar, Google Calendar,
 ...
 *
 * The season data (season.json calendar entries) gives us a race *date* but no
 * confirmed lights-out *time*, so by default each race is written as an all-day
 * VEVENT on race day (DTSTART;VALUE=DATE). When a real start time is known it
 * can be passed via `startUtc` and a timed, 2-hour UTC event is emitted
 * instead — no time is ever fabricated.
 *
 * Output conforms to RFC 5545: CRLF line endings, 75-octet line folding,
 * TEXT escaping, and stable per-race UIDs (so re-adding updates the existing
 * event instead of duplicating it).
 */

/** Minimal shape of a race, matching season.json `calendar[*]` entries. */
export interface CalendarRace {
  round: number;
  /** e.g. "Australian Grand Prix" */
  name: string;
  /** e.g. "Albert Park" */
  circuit?: string;
  /** ISO date, "YYYY-MM-DD" */
  ...
  country?: string;
  laps?: number;
  /** Optional confirmed lights-out time as an ISO instant, e.g. "2026-03-08T05:00:00Z". */
  startUtc?: string;
}
...
const PRODID = "-//RaceIQ//F1 Race Calendar//EN";
...
function stableUid(race: CalendarRace, season?: number): string {
  ...
  return `f1-${yr}-round-${race.round}@raceiq`;
}
...
function buildVeventLines(race: CalendarRace, opts: IcsOptions): string[] {
  ...
  lines.push(`SUMMARY:${escapeText(`${race.name} — F1`)}`);
...
export function buildRaceIcs(race: CalendarRace, opts: IcsOptions = {}): string {
  const name = `${race.name}${opts.season ? ` (F1 ${opts.season})` : " (F1)"}`;
...
export function buildSeasonIcs(
  ...
  const name = `Formula 1${opts.season ? ` ${opts.season}` : ""} Season — RaceIQ`;
...
export function icsFilename(race?: CalendarRace, season?: number): string {
  if (!race) return `f1-${season ?? "season"}-calendar.ics`;
  const slug = race.name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/(^-|-$)/g, "");
  return `${slug || `round-${race.round}`}-f1${season ? `-${season}` : ""}.ics`;
}
