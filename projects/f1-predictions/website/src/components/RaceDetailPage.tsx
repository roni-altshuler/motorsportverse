...
import { RoundData, SeasonData, DriverStanding, ProbabilityRoundData } from "@/types";
...
function bestQualifyingTime(row: {
  q1?: string | null;
  q2?: string | null;
  q3?: string | null;
  time?: string | null;
...
export default function RaceDetailPage({ round }: Props) {
  ...
  // Probability-layer file for the driver-comparison module (optional).
  useEffect(() => {
    let active = true;
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="card p-6 mb-8">
            ...
            <p style={{ color: "var(--text-muted)" }}>
              {isPostponed
                ? seasonRace.statusNote ||
                  "This Grand Prix has been postponed. Predictions and race-comparison reporting will resume once a revised race date is confirmed."
          ...
          <Link href="/calendar" className="text-f1-red font-bold hover:underline">
            ← Back to Calendar
          </Link>
        </motion.div>
      </div>
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-20">
        <div className="empty-state-hud">
          <p className="eyebrow mb-4">Off the calendar</p>
          <h1 className="display-md mb-4">Race Not Found</h1>
          <p className="body-md mb-8" style={{ color: "var(--text-muted)" }}>
            Round {round} is outside the {seasonYear} calendar.
          </p>
  ...
  const defaultWeekendSession =
    weekendSessions.find((session) => session.rows.length > 0) || weekendSessions[0] || null;
  const activeSession =
    weekendSessions.find((session) => session.key === activeWeekendSession) ||
    defaultWeekendSession;
  const loadedWeekendSessions = weekendSessions.filter((session) => session.rows.length > 0);
  const actualRows = data.actualResults
    ? Object.entries(data.actualResults).sort((a, b) => a[1] - b[1])
    : [];
  ...
  const comparisonChartData = actualRows
    ...
    .filter(
      (x): x is { driver: string; predicted: number; actual: number; delta: number } => x !== null,
    )
    .slice(0, 12);
  const teamErrorBars = Array.from(
    ...
    .slice(0, 10);
  const speedTrapChartData = (data.telemetryData?.speedTraps || []).slice(0, 10).map((st) => ({
    driver: st.driver,
    speed: st.speedKmh,
    sector: `S${st.sector}`,
  }));
  const pitImpactChartData = (data.telemetryData?.pitStopImpact || []).slice(0, 12).map((p) => ({
    ...
    <div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-8">
        ...
        <HUDHeader
          ...
          sprint={data.sprint}
          liveLabel={isPredictionPublished ? liveMeta?.label : "Awaiting Qualifying"}
          liveBadgeVariant={isPredictionPublished ? toneVariant(liveMeta?.tone) : "muted"}
          weather={
            data.weatherData
              ? {
        ...
        {/* Race-weekend state timeline: Preview → Qualifying → Race → Graded. */}
        <WeekendTimeline
          predictionPhase={data.predictionPhase}
          hasActualResults={actualRows.length > 0}
          lifecycle={seasonRace ? getRoundLifecycle(seasonRace, true, actualRows.length > 0) : null}
        />
        ...
        {isPredictionPublished ? (
          ...
          <div className="mb-8 p-6 sm:p-8 rounded-[var(--radius-card)] border border-[color:var(--hairline)] bg-[color:var(--surface-card)]">
            ...
            <p className="body-sm max-w-2xl" style={{ color: "var(--text-muted)" }}>
              The race prediction for {data.name} publishes after qualifying is complete. Qualifying
              pace is the strongest single-lap signal of race competitiveness — the model holds back
              until it has those lap times to ensure the published forecast reflects the freshest
            ...
            <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
              <div>
                ...
                <h2 className="text-2xl font-black" style={{ color: "var(--text)" }}>
                  {actualRows.length > 0
                    ? "Prediction vs Official Outcome"
                    : "Pre-Race Prediction Briefing"}
                </h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div key={`pred-${entry.driver}`} className="report-row">
                      ...
                      <div className="text-right">
                        <p className="text-sm font-mono" style={{ color: "var(--text-muted)" }}>
                          {entry.gap === "LEADER" ? "Projected leader" : `+${entry.gap}s`}
                        </p>
              ...
              <div className="report-panel">
                <p
                  className="text-xs font-semibold uppercase tracking-[0.24em] mb-3"
                  style={{
                    color: actualRows.length > 0 ? "var(--accent-positive)" : "var(--accent-info)",
                  }}
                >
                ...
                {actualRows.length > 0 ? (
                      ...
                      return (
                        <div key={`actual-${driver}`} className="report-row">
                          <div className="flex items-center gap-3">
                            ...
                            <div
                              className="team-color-bar h-8"
                              style={{ backgroundColor: predicted?.teamColor || "#888" }}
                            />
                            <div>
                              <p className="font-bold" style={{ color: "var(--text)" }}>
                                {predicted?.driverFullName || driver}
                              </p>
                              <p className="text-xs" style={{ color: "var(--text-muted)" }}>
                                {predicted?.team || "Official result"}
                              </p>
                            </div>
                          </div>
                          <p
                          ...
                          >
                            ...
                            {delta == null
                              ...
                              : delta === 0
                                ? "Exact match"
                                : delta > 0
                                  ? `${delta} better than predicted`
                                  : `${Math.abs(delta)} worse than predicted`}
                          </p>
                        </div>
                      );
                    })}
                  ...
                  <div className="report-panel-muted">
                    Official results will appear here automatically after the Grand Prix weekend
                    once the post-race workflow syncs the classified order.
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 mt-4">
                <div className="metric-card">
                  ...
                  <p className="text-xl font-black" style={{ color: "var(--text)" }}>
                    {data.predictionInsights.mostLikelyWinner}
                  </p>
                  {data.predictionInsights.winnerProbability != null && (
                    <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
                      {data.predictionInsights.winnerProbability.toFixed(1)}% projected win
                      probability
                    </p>
                  )}
                </div>
                <div className="metric-card">
                  ...
                  <p className="text-xl font-black" style={{ color: "var(--text)" }}>
                    {data.predictionInsights.closestBattle.drivers.join(" vs ")}
                  </p>
                  <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
                    {data.predictionInsights.closestBattle.gap.toFixed(3)}s projected gap
                  </p>
                </div>
                <div className="metric-card">
                  <p className="text-xs" style={{ color: "var(--text-muted)" }}>
                  ...
                  <p className="text-xl font-black" style={{ color: "var(--accent-positive)" }}>
                ...
                <div className="metric-card">
                  ...
                  <p className="text-xl font-black" style={{ color: "var(--text)" }}>
                    {data.metrics.avgUncertainty?.toFixed(2) ?? "—"}
                  </p>
                </div>
              </div>
            )}
          ...
          >
            <div className="weekend-results-shell p-6 sm:p-7">
              <div className="flex flex-wrap items-start justify-between gap-4 mb-5">
                <div>
                  ...
                  <h3 className="text-2xl font-black" style={{ color: "var(--text)" }}>
                    Session Results Explorer
                  </h3>
                  <p className="text-sm mt-1 max-w-3xl" style={{ color: "var(--text-muted)" }}>
                    Review the full Grand Prix weekend timeline: sprint qualifying, sprint race,
                    Grand Prix qualifying, and the race result as soon as official or timing-backed
                    data is available.
                  </p>
                </div>
                <div className="data-freshness-meta">
                  <span>
                    {loadedWeekendSessions.length}/{weekendSessions.length || (data.sprint ? 4 : 2)}{" "}
                    sessions loaded
                  </span>
              ...
              {weekendSessions.length > 0 ? (
                  ...
                  {activeSession && (
                    <div className="session-detail-card">
                      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                        <div>
                          <h4 className="text-lg font-black" style={{ color: "var(--text)" }}>
                            {activeSession.label}
                          </h4>
                          <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
                        ...
                        <Badge variant={toneVariant(sessionStatusTone(activeSession.status))}>
                          {activeSession.rows.length
                            ? `${activeSession.rows.length} classified`
                      ...
                      {activeSession.rows.length > 0 ? (
                        <>
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
                            {activeSession.rows.slice(0, 3).map((row) => (
                              <div
                                key={`session-podium-${activeSession.key}-${row.driver}`}
                                className="session-podium-card"
                              >
                                <span
                                  className={`position-badge ${row.position === 1 ? "p1" : row.position === 2 ? "p2" : row.position === 3 ? "p3" : "points"}`}
                                >
                                  P{row.position}
                                </span>
                                <div
                              ...
                              <tbody>
                                {activeSession.rows.map((row) => (
                                  <tr key={`${activeSession.key}-${row.driver}`}>
                                    <td>
                                      <span
                                        className={`position-badge ${row.position <= 3 ? `p${row.position}` : row.position <= 10 ? "points" : "no-points"}`}
                                      >
                                        P{row.position}
                                      </span>
                                    </td>
                        ...
                        <div className="report-panel-muted">
                          {activeSession.note ||
                            "This session has not been published yet. The automated data pipeline will populate this tab once the result appears upstream."}
                        </div>
                      )}
                    </div>
                  )}
                ...
                <div className="report-panel-muted">
                  Weekend session tabs will appear after the export pipeline refreshes this round
                  with live qualifying, sprint, and race result metadata.
                </div>
              )}
            </div>
          </motion.div>
                  ...
                  <div className="card p-6">
                        ...
                        {data.accuracy?.accuracy_pct != null && (
                          <span
                            ...
                            style={{
                              ...
                              border:
                                "1px solid color-mix(in srgb, var(--accent-live) 30%, transparent)",
                            }}
                          >
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
                      <div className="metric-card">
                        ...
                        <p className="text-xl font-black" style={{ color: "var(--text)" }}>
                          {data.accuracy?.podium_hits ?? 0}
                          <span
                            className="text-sm font-semibold"
                            style={{ color: "var(--text-muted)" }}
                          >
                            /{data.accuracy?.podium_total ?? 3}
                          </span>
                        </p>
                        {data.accuracy?.podium_accuracy_pct != null && (
                          <p className="text-[10px]" style={{ color: "var(--text-muted)" }}>
                            {data.accuracy.podium_accuracy_pct}% of podium
                          </p>
                        )}
                      </div>
                      <div className="metric-card">
                        ...
                        <p className="text-xl font-black" style={{ color: "var(--text)" }}>
                          {data.accuracy?.points_hits ?? 0}
                          <span
                            className="text-sm font-semibold"
                            style={{ color: "var(--text-muted)" }}
                          >
                            /{data.accuracy?.points_total ?? 10}
                          </span>
                        </p>
                        {data.accuracy?.points_accuracy_pct != null && (
                          <p className="text-[10px]" style={{ color: "var(--text-muted)" }}>
                            {data.accuracy.points_accuracy_pct}% of points
                          </p>
                        )}
                      </div>
                      <div className="metric-card">
                        <p className="text-xs" style={{ color: "var(--text-muted)" }}>
                          Mean Error (finishers)
                        </p>
                        <p className="text-xl font-black" style={{ color: "var(--text)" }}>
                          {data.accuracy?.mean_position_error_classified ??
                            data.accuracy?.mean_position_error ??
                            "-"}
                        </p>
                        {data.accuracy?.mean_position_error != null &&
                          data.accuracy?.mean_position_error_classified != null && (
                            <p className="text-[10px]" style={{ color: "var(--text-muted)" }}>
                              {data.accuracy.mean_position_error} all drivers
                            </p>
                          )}
                      </div>
                      <div className="metric-card">
                        ...
                        <p className="text-xl font-black" style={{ color: "var(--text)" }}>
                          {data.accuracy?.within_3_positions ?? 0}
                          <span
                            className="text-sm font-semibold"
                            style={{ color: "var(--text-muted)" }}
                          >
                            /{data.accuracy?.total_drivers ?? actualRows.length}
                          </span>
                        </p>
                      </div>
                    </div>
                    {(data.accuracy?.dnf_count != null || data.circuitVolatility) && (
                      ...
                      >
                        {data.accuracy?.dnf_count != null && data.accuracy.dnf_count > 0 && (
                          <>
                            Finisher accuracy excludes{" "}
                            <strong style={{ color: "var(--text)" }}>
                              {data.accuracy.dnf_count}
                            </strong>{" "}
                            retirement{data.accuracy.dnf_count === 1 ? "" : "s"}/DNS — outcomes
                            driven by reliability and incidents, not race-pace prediction.{" "}
                          </>
                        )}
                        {data.circuitVolatility && (
                          <>
                            Circuit volatility{" "}
                            <strong style={{ color: "var(--text)" }}>
                              {Math.round(data.circuitVolatility.volatilityScore * 100)}%
                            </strong>{" "}
                            (expected deviation from pace order;{" "}
                            {Math.round(data.circuitVolatility.safetyCarProbability * 100)}%
                            safety-car risk).
                          </>
                        )}
                      </p>
                      ...
                      <div className="mt-6 space-y-4">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                          ...
                          <div className="metric-card">
                            ...
                            <p className="text-xl font-black" style={{ color: "var(--text)" }}>
                              {gpReport.medianError.toFixed(1)}
                            </p>
                          </div>
                        </div>
                              ...
                              {gpReport.biggestMisses.slice(0, 5).map((miss) => (
                                ...
                                >
                                  ...
                                  <span
                                    ...
                                    style={{ color: "var(--accent-live)" }}
                                  >
                                    Pred P{miss.predicted} vs Actual P{miss.actual} (
                                    {miss.delta > 0 ? `+${miss.delta}` : miss.delta})
                                  </span>
                                </div>
                              ))}
                            </div>
              ...
              >
                ...
                <div className="card p-6 sm:p-8">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                    {[
                      ...
                      {
                        label: "Safety Car",
                        value: `${Math.round((data.circuitInfo.safetyCarLikelihood || 0.4) * 100)}%`,
                        bar: data.circuitInfo.safetyCarLikelihood || 0.4,
                        barColor:
                          (data.circuitInfo.safetyCarLikelihood || 0.4) > 0.6
                            ? "var(--accent-info)"
                            : "var(--accent-positive)",
                      },
                      { label: "Altitude", value: `${data.circuitInfo.altitudeM || 0} m` },
                      ...(data.sprint
                        ? [{ label: "Sprint Laps", value: data.sprintLaps || 0 }]
                        : []),
                    ].map((item) => (
                        ...
                        {"bar" in item && item.bar !== undefined && (
                          <div className="progress-bar mt-2">
                            <div
                              ...
                              style={{
                                width: `${(item.bar as number) * 100}%`,
                                background: item.barColor as string,
                              }}
                            />
                          </div>
                        )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      <div className="metric-card">
                        ...
                        <p className="text-2xl font-black" style={{ color: "var(--text)" }}>
                          {data.predictionInsights.poleToWinBias}%
                        </p>
                        <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
                          How strongly this circuit rewards track position in our forecast blend.
                        </p>
                      </div>
                      <div className="metric-card">
                        <p
                        ...
                        <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
                          {data.predictionInsights.winnerProbability?.toFixed(1) ?? "—"}% projected
                          win probability
                        </p>
                      </div>
                      <div className="metric-card">
                      ...
                      <div className="metric-card">
                        ...
                        <p className="text-2xl font-black" style={{ color: "var(--accent-info)" }}>
                          {data.predictionInsights.mediumConfidenceCount} /{" "}
                          <span style={{ color: "var(--accent-live)" }}>
                  ...
                  <div className="card p-6 sm:p-8">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                      <div className="metric-card">
                        ...
                        <p
                          ...
                          style={{
                            color:
                              data.weatherData.rainProbability > 0.5
                                ? "var(--accent-info)"
                                : data.weatherData.rainProbability > 0.25
                                  ? "var(--accent-info)"
                                  : "var(--accent-positive)",
                          }}
                        >
                        ...
                        <div className="progress-bar mt-2">
                          <div
                            className="progress-bar-fill"
                            style={{
                              width: `${data.weatherData.rainProbability * 100}%`,
                              background:
                                data.weatherData.rainProbability > 0.5
                                  ? "var(--accent-info)"
                                  : "var(--accent-positive)",
                      ...
                      <div className="metric-card">
                        ...
                        <p
                          ...
                          style={{
                            color:
                              data.weatherData.temperatureC > 35
                                ? "var(--accent-live)"
                                : data.weatherData.temperatureC < 15
                                  ? "var(--accent-info)"
                                  : "var(--text)",
                          }}
                        >
                      ...
                      {data.weatherData.humidity != null && (
                        <div className="metric-card">
                          ...
                          <p className="text-2xl font-black" style={{ color: "var(--text)" }}>
                            {Math.round(data.weatherData.humidity)}%
                          </p>
                        </div>
                      )}
                      {data.weatherData.windSpeedKmh != null && (
                        <div className="metric-card">
                          ...
                          <p
                            ...
                            style={{
                              color:
                                data.weatherData.windSpeedKmh > 30
                                  ? "var(--accent-info)"
                                  : "var(--text)",
                            }}
                          >
                      ...
                      {data.weatherData.cloudCover != null && (
                        <div className="metric-card">
                          ...
                          <p className="text-2xl font-black" style={{ color: "var(--text)" }}>
                            {Math.round(data.weatherData.cloudCover)}%
                          </p>
                        </div>
                      )}
                      {data.weatherData.precipitationMm != null &&
                        data.weatherData.precipitationMm > 0 && (
                          <div className="metric-card">
                            <p
                              className="text-xs font-medium mb-1"
                              style={{ color: "var(--text-muted)" }}
                            ...
                            >
                              {data.weatherData.precipitationMm.toFixed(1)} mm
                            </p>
                          </div>
                        )}
                    </div>
                    ...
                    {data.weatherData.source && (
                      <p className="mt-1 text-xs" style={{ color: "var(--text-muted)" }}>
                        Source:{" "}
                        {data.weatherData.source === "static"
                ...
                {data.telemetryData && (
                  <div className="card p-6 sm:p-8">
                                ...
                                >
                                  ...
                                  <td className="px-3 py-2 flex items-center gap-2">
                                    <div
                                      ...
                                      style={{ backgroundColor: st.teamColor }}
                                    />
                                  ...
                                  <td
                                    className="px-3 py-2 font-mono font-bold"
                                    style={{
                                      color: i === 0 ? "var(--accent-live)" : "var(--text)",
                                    }}
                                  >
                                    {st.speedKmh} km/h
                                  </td>
                    ...
                    {data.telemetryData.sectorTimes &&
                      data.telemetryData.sectorTimes.length > 0 && (
                        <div>
                          <h4
                            className="text-sm font-bold uppercase tracking-wider mb-3"
                            style={{ color: "var(--text-muted)" }}
                          ...
                          <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                              <thead>
                                <tr style={{ borderBottom: "1px solid var(--border)" }}>
                                  {["#", "Driver", "Team", "S1", "S2", "S3", "Ideal Lap"].map(
                                    (h) => (
                                      <th
                                        key={h}
                                        className="px-3 py-2 text-left text-xs font-bold uppercase tracking-wider"
                                ...
                                {data.telemetryData.sectorTimes.slice(0, 10).map((st, i) => (
                                  ...
                                  >
                                    ...
                                    <td
                                      ...
                                      style={{ color: "var(--text)" }}
                                    >
                                      {st.sector1.toFixed(3)}s
                                    </td>
                                    <td
                                      className="px-3 py-2 font-mono"
                                      style={{ color: "var(--text)" }}
                                    >
                                      {st.sector2.toFixed(3)}s
                                    </td>
                                    <td
                                      className="px-3 py-2 font-mono"
                                      style={{ color: "var(--text)" }}
                                    >
                                      {st.sector3.toFixed(3)}s
                                    </td>
                                    <td
                                      className="px-3 py-2 font-mono font-bold"
                                      style={{
                                        color: i === 0 ? "var(--accent-positive)" : "var(--text)",
                                      }}
                                    >
                                      {st.idealLap.toFixed(3)}s
                                    </td>
                                  </tr>
                                ))}
                              </tbody>
                    ...
                    <div className="space-y-3">
                      {stintTimeline.slice(0, 12).map((row) => {
                        const totalLaps = Math.max(
                          1,
                          row.stints.reduce((acc, s) => Math.max(acc, s.endLap), 0),
                        );
                        return (
                          ...
                          {trackStatusEvents.slice(0, 20).map((e, i) => (
                            ...
                            >
                              <span
                                ...
                                style={{ color: "var(--text-muted)", minWidth: 82 }}
                              >
                                {e.time || "--:--"}
                              </span>
                              <span
                  ...
                  <div className="card p-6 sm:p-8">
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr style={{ borderBottom: "1px solid var(--border)" }}>
                            {["Driver", "Team", "S1 Rank", "S2 Rank", "S3 Rank", "Overall"].map(
                              (h) => (
                                <th
                                  key={h}
                                  className="px-3 py-2 text-left text-xs font-bold uppercase tracking-wider"
                          ...
                          {sectorDominance.slice(0, 12).map((row) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <td
                                  className="px-3 py-2 font-mono"
                                  style={{ color: rankColor(row.sector1Rank) }}
                                >
                                  #{row.sector1Rank}
                                </td>
                                <td
                                  className="px-3 py-2 font-mono"
                                  style={{ color: rankColor(row.sector2Rank) }}
                                >
                                  #{row.sector2Rank}
                                </td>
                                <td
                                  className="px-3 py-2 font-mono"
                                  style={{ color: rankColor(row.sector3Rank) }}
                                >
                                  #{row.sector3Rank}
                                </td>
                                <td
                                  className="px-3 py-2 font-mono font-bold"
                                  style={{ color: rankColor(row.overallRank) }}
                                >
                                  #{row.overallRank}
                                </td>
                              </tr>
                ...
                <div className="card p-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    ...
                    <div className="metric-card">
                      ...
                      <p className="text-2xl font-black" style={{ color: "var(--text)" }}>
                        {data.metrics.trainingYears.join(", ")}
                      </p>
                    </div>
                    <div className="metric-card">
                      <p
                  ...
                  <div className="space-y-3">
                    {data.featureImportance.slice(0, 9).map((f, i) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className="w-40 sm:w-52 text-sm font-medium truncate"
                            style={{ color: "var(--text)" }}
                          >
                            {f.feature}
                          </div>
            ...
            <div className="deep-dive-section-body">
              ...
              the "more detail" view. */}
                ...
                {strategyData ? (
                  <div className="card p-6 sm:p-8">
                    ...
                    <p className="text-sm mb-4" style={{ color: "var(--text-muted)" }}>
                      Projected race time over {data.circuitInfo.laps} laps
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                      {Object.entries(strategyData).map(([name, d]: [string, any]) => {
                        ...
                        return (
                          <div
                            ...
                            style={isOptimal ? { border: "1px solid rgba(0,210,190,0.3)" } : {}}
                          >
                            {isOptimal && (
                              <span
                                className="absolute -top-2 left-3 px-2 py-0.5 text-xs font-bold rounded-full"
                                style={{
                  ...
                  <div className="empty-state-hud">
                    <p className="eyebrow mb-3">Pit strategy</p>
                    <p className="title-sm mb-2">No strategy simulation yet</p>
                    <p className="body-sm" style={{ color: "var(--text-muted)" }}>
                      Pit-window comparisons for this round publish with the next model run.
                    </p>
                  </div>
                )}
                ...
                {tyreDegData ? (
                          ...
                          <div key={compound} className="metric-card">
                            <div className="flex items-center gap-2 mb-3">
                              <div
                                ...
                                style={{ backgroundColor: compoundColors[compound] || "#888" }}
                              />
                  ...
                  <div className="empty-state-hud">
                    <p className="eyebrow mb-3">Tyre degradation</p>
                    <p className="title-sm mb-2">No compound analysis yet</p>
                    <p className="body-sm" style={{ color: "var(--text-muted)" }}>
                      Per-compound degradation curves for this round publish with the next model
                      run.
                    </p>
                  </div>
                )}
            ...
            <div className="deep-dive-section-body">
              ...
              >
                ...
                <HUDPanel
                  ...
                  bodyClassName="p-4 sm:p-5"
                >
                  <ChartContainer fallbackSrc={null} fallbackAlt="Head-to-head edges" height={420}>
        ...
        >
          {round > 1 ? (
            <Link
              href={`/race/${round - 1}`}
              className="group text-f1-red font-bold transition-colors inline-flex items-center gap-1 hover:underline"
            >
            ...
            <Link
              href="/calendar"
              className="group font-medium inline-flex items-center gap-1"
              style={{ color: "var(--text-muted)" }}
            >
              <span className="group-hover:-translate-x-1 transition-transform inline-block">
                ←
