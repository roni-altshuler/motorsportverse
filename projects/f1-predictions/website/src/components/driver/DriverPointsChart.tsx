 ...
 *
 * Sourced from standings.json `pointsHistory` (cumulative points per round) via
 * `pointsProgression()`. Team-coloured area line; the tooltip surfaces both the
 * running total and the points scored in that round. Static, non-interactive
 * beyond hover — no animation loop, so it stays cheap on a static export.
 */
import {
  Area,
  AreaChart,
...
// so the codebase types custom tooltips locally (see CalibrationPanel).
