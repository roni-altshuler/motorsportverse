...
/**
 * ShareButton — lightweight share cluster for the race-detail header.
 *
 ...
 *
 * The page already ships a per-round OG image at `public/og/round_NN.png`,
 * so any surface that unfurls the shared URL renders that card automatically —
 * this button only needs to hand over the page URL (read lazily from
 * `window.location` so it stays correct under static export / base paths).
 */
import { useCallback, useState, useSyncExternalStore } from "react";

interface ShareButtonProps {
  /** Headline used by the native sheet + tweet text, e.g. the Grand Prix name. */
  title: string;
...
export default function ShareButton({ title, text, className }: ShareButtonProps) {
  ...
  const shareText = text ?? `${title} — race prediction & win-probability model`;
