 ...
 * round, coloured by status. Completed rounds are solid (IndyCar red);
 * upcoming rounds are hollow; the Indy 500 carries a marker tick under
          ...
          {calendar.map((race, i) => {
            ...
            return (
              <motion.div
                ...
                title={`R${race.round}: ${race.name}${race.isIndy500 ? " (Indy 500)" : ""}`}
              >
                <Link
                  ...
                  style={{
                    ...
                    outline: race.isIndy500
                      ? "1px solid color-mix(in srgb, var(--accent) 55%, transparent)"
                      : undefined,
                    outlineOffset: race.isIndy500 ? "2px" : undefined,
                    boxShadow: isNext ? "0 0 12px var(--accent)" : undefined,
                  }}
                  aria-label={`Round ${race.round}: ${race.name}${race.isIndy500 ? " (Indy 500)" : ""}`}
                />
                {isNext && !reduced && (
                  <motion.span
                    aria-hidden
