...
/**
 * Race-weekend narrative card — IndyCar adaptation of the F1 flagship's
 ...
 * editorial work.
 *
 * The rules are rebuilt around the data the IndyCar export actually has (there
 * is no weather feed and no caution model in the snapshot — those F1 rules are
 ...
 *   2. Track framing — the three IndyCar archetypes read very differently
 *      (oval pack racing vs permanent road courses vs street-circuit walls).
 *   3. DNF risk — the model carries a first-class per-driver DNF hazard tuned
 *      to oval risk; when the favourite's hazard is elevated, say so.
 *   4. Stakes — Indy 500 framing on the crown-jewel round; otherwise the
 *      championship read for upcoming rounds.
...
import type { RoundDetail, TitleOdds } from "@/types/indycar";
...
const OPEN_RACE_P = 0.1; // no clear favourite below this win probability (27-car fields)
...
function buildBullets(
    ...
    if (typeof top.pDnf === "number" && top.pDnf >= HIGH_DNF_P && !round.completed) {
      out.push({
        ...
        text: `${top.name} carries a ${(top.pDnf * 100).toFixed(0)}% retirement hazard here — ${
          round.trackType === "oval"
            ? "oval wrecks don't care who was fastest"
  ...
  // 2. Track framing — the three IndyCar archetypes.
  ...
  if (blurb) {
    out.push({
      ...
      text: `${round.venueName} is ${
        round.trackType === "oval" ? "an" : "a"
      } ${trackTypeLabel(round.trackType).toLowerCase()} — ${blurb}.`,
    });
  }

  // 4. Stakes — Indy 500 framing on the crown jewel beats the season-long read.
  if (isIndy500 && !round.completed) {
    out.push({
      label: "INDY 500",
      tone: "live",
      text: `The Indianapolis 500 — a 33-car field with one-off entries, and the biggest single prize on the calendar.`,
    });
...
export default function RaceNarrativeCard({
  ...
  const bullets = buildBullets(round, championship, isIndy500);
