...
import EnginePanel from "@/components/standings/EnginePanel";
...
// season's indycar.json (client-side). Uses each row's exported pointsHistory —
// the same fallback the server page's series builder uses.
function propsFromSeasonData(data: IndycarData): StandingsPageProps {
  const TOP_DRIVERS = 6;
  const TOP_TEAMS = 5;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 section-bugatti">
      <div className="mb-16 text-center">
        <p className="eyebrow mb-4">NTT IndyCar Series · Championship</p>
        <h1 className="display-xl mb-4">{season} Standings</h1>
        <p className="body-md text-[color:var(--body)] max-w-xl mx-auto">
          Who is winning the championship — and who can still catch the leader on
      ...
      <div className="flex justify-center gap-2 mb-10 relative">
        {(
          [
            ...
            { key: "wdc" as Tab, label: "Title Race", href: "/standings?tab=wdc" },
          ] as const
        ).map((tab) => (
          <a
            key={tab.key}
            ...
            <div className="card p-6">
              <div className="flex items-baseline justify-between gap-3 flex-wrap">
                ...
                <p className="text-[11px] font-mono uppercase tracking-[0.14em] text-[color:var(--text-muted)]">
                  projection to the season finale
                </p>
              </div>
              <ProgressionChart
                series={driverSeries}
      ...
      {activeTab === "teams" && (
        <div className="space-y-8">
          ...
          {engines && engines.length > 0 && <EnginePanel engines={engines} />}
      ...
      {/* Title Race tab — championship forecast to the finale */}
      {activeTab === "wdc" && (
