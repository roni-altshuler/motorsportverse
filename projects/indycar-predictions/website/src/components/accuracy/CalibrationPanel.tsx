import type { CalibrationSummary } from "@/types/indycar";
...
/**
 * Renders calibration_summary.json — the honest calibration status the IndyCar
 * model earned on real rounds. Adapts the F1 flagship's calibration panel to
 * the IndyCar shape: applied flag, training-round count, the three track-type strata
 * the calibrator was fitted on (superspeedway / intermediate / short / road),
 * and the per-market observation counts. (The values in `perMarket` are sample
 * counts, not temperatures — labelled as such.)
 */
    ...
    <section className="mt-12">
        ...
        <span
          ...
          style={{
            background: summary.applied
              ...
              : "var(--surface-2)",
            color: summary.applied ? "var(--accent-f1-red-bright)" : "var(--ink-muted)",
            border: `1px solid ${
              summary.applied
      ...
      <div className="rounded-[var(--radius-lg)] border border-[var(--hairline)] bg-[var(--surface)] p-6">
        <p className="text-sm text-[var(--ink-muted)]">
          ...
          actually happen. The forecasts are tuned against the season&rsquo;s real
          classified results — separately for each of the four track types, because
          Talladega and Sonoma race nothing alike — so a stated 30% podium chance
          means roughly 3-in-10 over the long run.
        </p>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="rounded-[var(--radius-md)] border border-[var(--hairline)] p-4 sm:col-span-1 lg:col-span-2">
            ...
            {summary.strata && Object.keys(summary.strata).length > 0 && (
              <p className="mt-1 mb-1 flex flex-wrap gap-1.5">
                {Object.keys(summary.strata).map((k) => (
                  <span
                    key={k}
                    className="rounded-full border border-[var(--hairline-strong)] px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-[var(--ink-muted)]"
                  >
                    {`${k} stratum`}
                  </span>
                ))}
