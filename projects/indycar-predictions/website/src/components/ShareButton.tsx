...
/**
 * ShareButton — lightweight share cluster for standings / accuracy / race pages.
 *
 ...
 *
 * The button hands over the current page URL (read lazily from
 * `window.location` so it stays correct under static export / base paths).
 * Ported verbatim from the RaceIQ F1 flagship — styling flows entirely through
 * CSS custom properties, so it themes to IndyCar red automatically.
 */
import { useCallback, useState, useSyncExternalStore } from "react";

interface ShareButtonProps {
  /** Headline used by the native sheet + tweet text, e.g. the race name. */
  title: string;
