...
// Client-side loaders for the IndyCar dataset. The Navbar and other client
...
import type { IndycarData, ProbabilitiesRound, RoundDetail } from "@/types/indycar";

import { useSeason } from "./SeasonProvider";
import { BASE_PATH } from "./seasons";

const cache = new Map<string, IndycarData | null>();
const inflight = new Map<string, Promise<IndycarData | null>>();

export function fetchIndycarData(base: string = BASE_PATH): Promise<IndycarData | null> {
  if (cache.has(base)) return Promise.resolve(cache.get(base) ?? null);
  const pending = inflight.get(base);
  if (pending) return pending;
  const p = fetch(`${base}/indycar.json`)
    .then((r) => (r.ok ? (r.json() as Promise<IndycarData>) : null))
    .then((d) => {
      cache.set(base, d);
...
/** indycar.json for a data root (defaults to the CURRENT season's root). */
export function useIndycarData(base: string = BASE_PATH): IndycarData | null {
  const [data, setData] = useState<IndycarData | null>(cache.get(base) ?? null);
  useEffect(() => {
    let active = true;
    fetchIndycarData(base).then((d) => {
      if (active) setData(d);
    });
    return () => {
      active = false;
...
/**
 * indycar.json for the season selected in the SeasonProvider. `isArchived` is true
 ...
 */
export function useSeasonIndycarData(): { data: IndycarData | null; isArchived: boolean } {
  const { basePath, year, index } = useSeason();
  const data = useIndycarData(basePath);
