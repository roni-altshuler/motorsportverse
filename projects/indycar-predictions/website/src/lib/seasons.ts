...
// FE has no fs-free lib/data.ts, so the data root constant lives here (safe to
 ...
 * index derived from indycar.json when the site predates multi-season support,
...
export async function fetchSeasonsIndex(): Promise<SeasonsIndex> {
  ...
  // Legacy fallback: derive the current season from indycar.json.
  let current = new Date().getFullYear();
  try {
    const res = await fetch(`${BASE_PATH}/indycar.json`);
    if (res.ok) {
