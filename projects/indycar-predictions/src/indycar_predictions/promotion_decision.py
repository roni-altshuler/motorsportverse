"""Promotion gate for IndyCar — thin CLI over motorsport_core.promotion.
...
(:mod:`.position_head`, A/B-gated by ``INDYCAR_USE_POSITION_HEAD`` and OFF by
default). Its walk-forward evidence is the
``forward_eval/position_model_ab.json`` artifact written by
``python -m indycar_predictions.forward_eval --position-model-ab``: per
...
Run:  python -m indycar_predictions.promotion_decision --season 2026
          [--allow-empty]
"""
...
def build_status(data_dir: Path) -> dict:
    ...
    return {
        ...
        "candidateFlag": "INDYCAR_USE_POSITION_HEAD",
