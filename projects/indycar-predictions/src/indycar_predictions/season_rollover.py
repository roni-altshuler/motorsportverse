"""Season rollover — archive a completed IndyCar season and start the next one.

Ported from the FE/NASCAR template (itself from the F1 flagship) and adapted
to IndyCar's conventions: calendar-year seasons, the season lives in
``config.py`` (committed calendar/history files + announced-calendar
overrides), results come from the committed ``data/history_<year>.json``
(which simply becomes another archive season file — no copying needed), and
the website data contract is produced by ``indycar_predictions.export`` into
``website/public/data/``.
  ...
  --auto             If the active season is COMPLETE (the history file
                     carries every calendar round) AND the announced next
                     season has BEGUN (its first race date has passed),
                     archive the active season and start the next one.
                     Otherwise it only refreshes seasons.json — safe to call
                     from the cron on every run.

All steps support --dry-run.

Run:  python -m indycar_predictions.season_rollover --auto [--dry-run]
"""
...
SEASON_DIRS = ["rounds", "probabilities", "forward_eval", "historical_backtest"]
...
def _first_race_date(announced: dict) -> date | None:
    """The announced season's opening race date."""
    raw = announced.get("first_race_date")
    if not raw:
        cal = announced.get("calendar") or []
        raw = cal[0].get("date") if cal else None
    try:
...
def season_complete(
    ...
    """True when the history file carries every calendar round."""
    snap_path = snapshot_dir / f"history_{year}.json"
    try:
        snap = json.loads(snap_path.read_text(encoding="utf-8"))
    except Exception:
        return False
    completed = len(snap.get("rounds", []))
    total = int(total_rounds or 0)
    if not total:
        cal_path = snapshot_dir / f"calendar_{year}.json"
        try:
            total = int(json.loads(cal_path.read_text(encoding="utf-8")).get("total_rounds", 0))
        except Exception:
...
def archive(
    ...
    *,
    ...
    # The canonical history file already lives in data/ as history_<year>.json
    # — it IS the archive format, no copy needed.
    ...
    *,
    ...
    if year != config._DEFAULT_SEASON and _load_announced(year, announced_dir) is None:
        raise SystemExit(
            ...
            f"`python -m indycar_predictions.bootstrap_next_season --year {year}` first."
        )
    print(f"Starting season {year} ({config.season_label(year)}) as the active season")
    if dry_run:
        print("   would write data/active_season.json, seed the new history file, "
              "clear round files, refresh seasons.json")
        return
    snapshot_dir.mkdir(parents=True, exist_ok=True)
    (snapshot_dir / "active_season.json").write_text(
        json.dumps({"season": year}, indent=2) + "\n", encoding="utf-8"
    )
    # Seed an empty history file for the new season (refresh appends to it).
    hist = snapshot_dir / f"history_{year}.json"
    if not hist.exists():
        hist.write_text(
            json.dumps(
                {
                    ...
                    "rounds": [],
                    "final_standings": [],
                    "notes": [f"seeded empty by season_rollover --start {year}"],
                },
                ensure_ascii=False,
                indent=2,
            )
    ...
    print(
        f"Season {year} is now active. Run `python -m indycar_predictions.refresh` and "
        "`python -m indycar_predictions.export` to populate it."
    )
...
def auto(
    *,
    ...
    """Roll over automatically once the active season finished AND the next
    began.

    Mirrors F1's ``--auto`` semantics: the finished season keeps serving as
    the live site through the off-season; the archive+start pair only fires
    when the announced next season's first race date has passed. Returns True
    iff a rollover happened.
    """
    announced_dir = announced_dir if announced_dir is not None else snapshot_dir / "announced_seasons"
    ...
    if announced is None:
        return _no_op(
            ...
            f"— run `python -m indycar_predictions.bootstrap_next_season`."
        )
