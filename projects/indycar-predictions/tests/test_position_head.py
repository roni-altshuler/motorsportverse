...
from indycar_predictions import config, model, position_head as ph
...
def test_extract_features_shape(real_source):
    fc = model.forecast_round(real_source, SEASON, 2, use_position_head=False)
    ...
    # Phoenix is an oval: the one-hots must say so.
    assert all(f["isOval"] == 1.0 and f["isStreet"] == 0.0 for f in feats.values())
...
def test_training_frame_is_prior_only(truncated_source):
    ...
    assert used == [1, 2, 3]
    assert X.shape[0] == len(y) > 60  # ~25 rows per round
