"""Refresh the committed FIA WEC snapshot, then re-export the website data.

Two steps, in order:

1. **Re-pull** (network) the current season from the official Al Kamel timing
   archive via the committed ingester (:mod:`wec_predictions.build_snapshot`),
   ...
   guard is inherent (each round is keyed by its Al Kamel event folder).

2. **Re-export** the website JSON + forward-eval from whatever snapshot is now on
   disk. IMPORTANT: :mod:`wec_predictions.config` resolves the active season (and
   ...
   :mod:`wec_predictions.export` and :mod:`wec_predictions.forward_eval` as
...
Run:  PYTHONPATH=src python -m wec_predictions.refresh [--season 2026]
...
def _run_module(module: str, args: list[str], season: int) -> int:
    ...
    ``config`` re-resolves against the snapshot on disk. ``WEC_SEASON_YEAR`` pins the
    season the child resolves to; PYTHONPATH is inherited from this process."""
    env = dict(os.environ)
    env["WEC_SEASON_YEAR"] = str(season)
...
def main() -> int:
    ...
    rc = _run_module("wec_predictions.export", [], args.season)
    if rc != 0:
    ...
    if not args.skip_forward_eval:
        rc = _run_module(
            "wec_predictions.forward_eval", ["--season", str(args.season), "--allow-empty"],
            args.season,
        )
        if rc != 0:
