"""The unique FIA WEC model behaviours: valid per-class markets, leakage-safety,
and the never-ship-worse gate against the season-form baseline.

Endurance is multi-class, so a round is one race *per class* and every market is
keyed by class. Kept fast (OMP_NUM_THREADS=1, small n_samples).
"""
...
from wec_predictions import config, forward_eval, model
from wec_predictions.datasource import WecDataSource


@pytest.fixture
def source():
    return WecDataSource()
...
def test_model_not_worse_than_season_form_baseline(source, monkeypatch):
    """Aggregate win+podium Brier over the modern three-class era (2024-2026) must
    not be worse than the season-form baseline by more than a tiny Monte-Carlo
    tolerance — the honest never-ship-worse property the model was tuned for.
