"""Contract tests for FIA World Endurance Championship.
...
from wec_predictions.sources import snapshot
...
def test_datasource_implements_the_core_contract():
    assert issubclass(WECDataSource, CoreDataSource)
...
def test_fit_signature_matches_the_contract():
    ...
    assert list(inspect.signature(WECPredictor.fit).parameters) == [
...
def test_no_snapshot_means_an_empty_calendar_not_an_invented_one():
    assert list(WECDataSource().calendar(config.DEFAULT_SEASON)) == []


def test_no_snapshot_means_an_empty_grid():
    assert list(WECDataSource().grid(config.DEFAULT_SEASON, 1)) == []


def test_no_snapshot_means_no_results():
    assert dict(WECDataSource().results(config.DEFAULT_SEASON, 1)) == {}


def test_an_empty_grid_produces_an_empty_order_not_a_guess():
    forecast = WECPredictor().predict(WECDataSource(), config.DEFAULT_SEASON, 1)
...
def test_strict_mode_fails_loudly_instead_of_returning_empty():
    ...
    with pytest.raises(snapshot.SnapshotUnavailable) as excinfo:
        WECDataSource(strict=True).calendar(config.DEFAULT_SEASON)
...
def test_fit_only_sees_prior_rounds():
    predictor = WECPredictor()
    predictor.fit(WECDataSource(), config.DEFAULT_SEASON, upto_round=5)
