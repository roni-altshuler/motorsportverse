// TypeScript mirror of the JSON produced by the WEC predictions pipeline
// (wec.json + per-round rounds/probabilities, plus the calibration summary).
//
// This is the load-bearing data contract between the Python pipeline and the
// website. The WEC dataset's defining difference from every other MotorsportVerse
// sport is that endurance racing is MULTI-CLASS: several classes (Hypercar,
// LMGT3, …) race the same event simultaneously, each scored as its own
...
export interface ClassMeta {
  /** Stable class key, e.g. "HYPERCAR" | "LMGT3" | "LMP2". */
  key: string;
  /** Display label, e.g. "Hypercar". */
  label: string;
...
export interface CalendarRound {
  ...
  country: string | null;
  completed: boolean;
  /** Le Mans is the 24-hour crown jewel — flagged for special treatment. */
  isLeMans: boolean;
...
// --------------------------------------------------------------------------- //
// Season summary — public/data/wec.json
// --------------------------------------------------------------------------- //
export interface WecData {
  sport: string;
  season: number;
  generatedAt?: string;
