...
import { getRoundLifecycle, getRoundStatusMeta, useSeasonWrcData } from "@/lib/wrcClient";
...
/**
 * Inline "RaceIQ WRC" wordmark. Rendered as SVG (not an <img>) so the accent
 * always resolves to the live WRC-blue token in tokens.css — the chevron mark
 * and the "WRC" line pick up `--accent-f1-red` / its bright small-text variant.
 */
function Wordmark({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 300 80" className={className} role="img" aria-label="RaceIQ WRC">
 ...
 * WRC data contract (wrc.json). Hover-opening Rallies dropdown with a ~140ms grace
 * timeout, Standings dropdown (drivers / manufacturers / who-can-win) with computed
...
export default function Navbar() {
  ...
  const { data, isArchived } = useSeasonWrcData();
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center h-14 gap-2 sm:gap-4">
          ...
          {/* wordmark — RaceIQ WRC brand lockup (inline so it always tracks the accent token) */}
          <Link href="/" className="flex items-center group shrink-0" aria-label="RaceIQ WRC home">
                ...
                {standingsOpen && (
                  ...
                  >
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        { href: "/standings?tab=drivers", label: "Drivers", hint: "Per-crew totals" },
                        { href: "/standings?tab=teams", label: "Manufacturers", hint: "Marque-by-marque" },
          ...
          {nextRound && (
            <Link
              ...
              aria-label={`Next rally: ${nextRound.name}`}
            >
              <ShimmerButton
            ...
            >
              ...
              <nav className="flex flex-col gap-0 py-2">
                ...
                {calendar.length > 0 && completedRounds > 0 && (
                  <div className="pt-4">
                    <p className="eyebrow px-5 pt-4 pb-2">Rally Status</p>
                    {calendar
