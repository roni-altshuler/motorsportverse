...
/**
 * Bento mosaic snapshot of the WRC championship state. Ported from RaceIQ F1's
 * ChampionshipBento. WRC runs a single rally classification per round (one result
 * per round), so the removed short-format tile becomes a title-odds tile sourced
 * from the drivers' championship odds, and the Next Round tile carries the rally's surface
 * — WRC's defining variable. The accuracy tile uses the season podium hit-rate,
 * since WRC has no single headline accuracy percentage like F1.
 */
export default function ChampionshipBento({
  ...
  const constructorLeader = manufacturerStandings[0];
  ...
  return (
    <BentoGrid className="grid-cols-2 md:grid-cols-4 auto-rows-[12rem] gap-3 sm:gap-4">
        ...
        <div className="flex h-full flex-col p-6">
          <div className="flex items-baseline justify-between mb-4">
            <div>
              ...
              <h3 className="title-md text-[color:var(--ink)]">Crews · Top 5</h3>
            </div>
            <Link
      ...
      {/* Manufacturers top 3 — 2x1 */}
      <BentoCard className="col-span-2 row-span-1 md:col-span-2 md:row-span-1">
        <div className="flex h-full flex-col p-5">
          <div className="flex items-baseline justify-between mb-2">
            <div>
              <p className="eyebrow mb-1">Manufacturers</p>
              <h3 className="title-md text-[color:var(--ink)]">Manufacturer Standings</h3>
            </div>
            <Link
          ...
          <div className="flex-1 grid grid-cols-3 gap-2 items-end">
            {manufacturerStandings.slice(0, 3).map((t, i) => (
              <div
                key={t.team}
                data-team={t.team}
          ...
          <div>
            ...
            {nextRace ? (
              <>
                ...
                <span
                  className="surface-chip mt-2"
                  data-surface={nextRace.surface}
                  style={
                    {
                      "--surface-color": resolveSurfaceColor(
                        nextRace.surface,
                        nextRace.surfaceColor,
                      ),
                    } as React.CSSProperties
                  }
                >
                  {surfaceLabel(nextRace.surface)}
                </span>
              </>
            ) : (
          ...
          {driverLeader ? (
            <div className="flex flex-1 items-end gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="caption-uppercase text-[10px] tracking-[0.16em] text-[color:var(--muted)] truncate">
                  {driverLeader.team} · {constructorLeader?.team ?? "—"} leads manufacturers
                </p>
              </div>
