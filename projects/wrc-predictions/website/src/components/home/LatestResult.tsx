...
interface LatestResultProps {
  /** Top-N rally finishers (already mapped + sorted). */
  results: ResultRow[];
}

/**
 * "Latest Official Result" — the most recent completed round's rally result.
 * Ported from RaceIQ F1's Latest Official Result table and adapted to WRC's data:
 * a rally has a single classification (one result per round, no grid, no
 * predicted times). Fed plain rows as props (never the fs loader). Reveal is
 * animate-on-mount with a reduced-motion fallback so content is never
 * permanently invisible.
 */
export default function LatestResult({ results }: LatestResultProps) {
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[color:var(--hairline)]">
              ...
              <th className="px-2 py-3 text-left eyebrow">Crew</th>
              <th className="px-2 py-3 text-left eyebrow hidden sm:table-cell">Manufacturer</th>
