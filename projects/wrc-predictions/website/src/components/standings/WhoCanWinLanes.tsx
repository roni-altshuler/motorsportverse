...
import type { TitleOdds } from "@/types/wrc";
...
/**
 * Crew title-race forecast lanes (port of the RaceIQ F1 WhoCanWinLanes).
 * Each lane shows the crew portrait + manufacturer, championship-win probability
 * (bar + %), current → projected points, and the projected range (P10–P90).
 * Sourced directly from WRC's `championship[]` (TitleOdds). Crews below the
  ...
  return (
    <div className="space-y-6">
        ...
        {eliminated.length > 0 && (
          <details className="mt-6 group">
            <summary className="cursor-pointer eyebrow text-[color:var(--text-muted)] hover:text-[color:var(--text)] transition-colors">
              Mathematically out · {eliminated.length} crew
      ...
      <p className="text-xs text-[color:var(--text-muted)]">
        Title odds are projected from each crew&apos;s current form across the remaining
        rallies, applying the WRC points system and accounting for retirement risk.
      </p>
    </div>
  );
}
