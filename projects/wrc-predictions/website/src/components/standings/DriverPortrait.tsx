...
/**
 * Circular crew portrait — ported from RaceIQ F1.
 * WRC ships no crew photography, so this renders a team-tinted conic backdrop
 * with a diagonal stripe + the 3-letter code. If a caller ever passes an
 * explicit `headshotUrl` it is shown instead (with a graceful fallback on load
 * error); otherwise the polished initials avatar IS the portrait. No image is
 * ever requested unless a URL is supplied, so nothing can 404.
 */
...
export default function DriverPortrait({
  ...
  // WRC has no headshots directory, so we never synthesise a `/headshots/*` path
  // (that would 404). Only an explicitly supplied URL is loaded.
  const resolvedSrc = resolveHeadshotSrc(headshotUrl ?? null);
