...
function teamInitials(team: string): string {
  // Both short crew marques ("Toyota") and long manufacturer entries
  // ("HYUNDAI SHELL MOBIS WORLD RALLY TEAM") resolve to a stable 3-letter mark
  // off the leading word, so the chip never overflows regardless of name length.
...
/**
 * WRC manufacturer / crew mark. No logo assets ship, so this renders a polished
 ...
 * through as the ring; a uniform light field keeps the initials legible. Long
 * manufacturer names collapse to their leading-word initials so the tile stays
 * clean; the full name is always shown as text alongside the badge.
 */
  ...
  return (
    ...
    >
      <span
        ...
        style={{
          fontSize: Math.max(9, Math.round(size * (isCard ? 0.32 : 0.3))),
          color: "#111",
          lineHeight: 1.1,
          textAlign: "center",
        }}
