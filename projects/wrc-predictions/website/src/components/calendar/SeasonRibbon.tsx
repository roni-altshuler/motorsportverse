 ...
 * round. Completed rounds are solid and tinted by that rally's SURFACE colour
 * (gravel / tarmac / snow) so the season's surface rhythm reads at a glance;
 * upcoming rounds are hollow. The next round pulses + glows in the WRC accent.
 * Each dot links to the round's detail page. The pulse honours
      ...
      <div className="relative overflow-x-auto">
        <div className="flex items-center gap-1 min-w-max px-1 pb-2">
          {calendar.map((race, i) => {
            ...
            // Completed dots take that rally's surface colour; upcoming stay hollow.
            const surfaceHue = race.surfaceColor || "var(--accent)";
            const fill = completed ? surfaceHue : "transparent";
            const border = completed ? surfaceHue : "var(--hairline-strong)";
            return (
              <motion.div
                key={race.round}
                ...
                title={`R${race.round}: ${race.name} · ${surfaceLabel(race.surface)}`}
              >
                <a
