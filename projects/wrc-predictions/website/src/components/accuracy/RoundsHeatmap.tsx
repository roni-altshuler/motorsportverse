...
import type { ForwardEvalRound, RaceAccuracy } from "@/types/wrc";

/**
 * Grid heatmap of per-round prediction accuracy (WRC port of the RaceIQ F1
 * flagship's RoundsHeatmap). One cell per scheduled round, colour-encoded by a
 * podium-weighted accuracy band derived from the rally classification. Click
 * opens a modal with the round breakdown and a link to the rally detail page.
 * Empty / not-yet-scored rounds render as a hairline placeholder.
 *
 * WRC has no single per-round "accuracyPct" like F1's season tracker, so the
 * score is derived honestly from the rally RaceAccuracy:
 *   0.6·(podium hits / 3) + 0.4·(within-5 order rate), re-normalised when a
 * component is missing — never fabricated.
 */

export function rallyAccuracyPct(a: RaceAccuracy | undefined): number | null {
...
// red as a semantic "miss" signal (red is semantic here, not the WRC brand).
...
export default function RoundsHeatmap({ rounds, totalRounds }: RoundsHeatmapProps) {
  ...
  const activePct = active ? rallyAccuracyPct(active.rally) : null;
    ...
    <>
      ...
      >
        {cells.map(({ round }) => {
          ...
          const pct = rallyAccuracyPct(entry?.rally);
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                ...
                <div className="metric-card text-center">
                  ...
                  <p className="font-mono font-tabular font-black text-3xl" style={{ color: "var(--text)" }}>
                    {active.rally.mean_position_error != null ? (
                      <NumberTicker value={active.rally.mean_position_error} decimalPlaces={1} />
                    ) : (
                      "—"
                    )}
              ...
              <div className="flex justify-between text-xs mb-5" style={{ color: "var(--text-muted)" }}>
                <span>
                  ...
                  <span style={{ color: "var(--text)" }}>
                    {active.rally.winner_hit == null ? "—" : active.rally.winner_hit ? "✓ hit" : "miss"}
                  </span>
                </span>
                <span>
                  Podium hits:{" "}
                  <span style={{ color: "var(--text)" }}>{active.rally.podium_hits ?? "—"}/3</span>
                </span>
              </div>
              <Link
