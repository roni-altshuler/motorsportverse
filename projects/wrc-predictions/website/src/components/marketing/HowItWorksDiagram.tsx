 ...
 * RaceIQ F1. WRC has no telemetry, weather or stage-timing feeds here, so the copy
 * is reworded around what WRC actually provides: per-round rally classifications,
 * the rally's surface, a crew-form model, and per-rally probabilities. Two
...
const STEPS: Step[] = [
  {
    Icon: Database,
    title: "WRC results",
    copy: "Rally classifications, the surface run (gravel, tarmac or snow) and championship standings, ingested after every round.",
  },
  {
    Icon: Cpu,
    title: "The model",
    copy: "A crew-form model — recent form, the rally's surface and championship position carry the signal. It re-trains as the season unfolds, then weighs every crew in the field.",
  },
  {
    Icon: Trophy,
    title: "Rally forecast",
    copy: "Probabilities for win, podium, top six and top ten — one classification for every rally.",
  },
];
  ...
  return (
    ...
    >
      ...
      <AnimatedBeam
        ...
        toRef={modelRef}
        duration={4}
        gradientStartColor="#0F62FE"
        gradientStopColor="#4589FF"
      />
      <AnimatedBeam
        containerRef={diagramRef}
        fromRef={modelRef}
        ...
        duration={4}
        delay={1}
        gradientStartColor="#4589FF"
        gradientStopColor="#6BA5FF"
      />
    </div>
  );
}
