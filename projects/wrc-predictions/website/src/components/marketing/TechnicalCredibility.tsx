 ...
 * WRC: reliability, freshness, reproducibility, openness, and the shared
 * MotorsportVerse core. Freshness comes in as a build-time prop; there is no
 * deep archive or telemetry feed here, so those claims are dropped.
 */
  ...
  const pillars: Pillar[] = [
    {
      ...
      body: "An automated pipeline folds in each rally's classification before the next round — recent crew form and the surface ahead, no manual intervention.",
    },
    {
      Icon: Clock,
    ...
    },
    {
      ...
      body: roundsGraded
        ? `Built on the same motorsport-core that powers RaceIQ F1, with ${roundsGraded} WRC ${roundsGraded === 1 ? "rally" : "rallies"} graded so far this season.`
        : "Built on the same motorsport-core that powers RaceIQ F1 — one engine, tuned for the WRC's drivers' and manufacturers' title fights.",
    },
  ];
