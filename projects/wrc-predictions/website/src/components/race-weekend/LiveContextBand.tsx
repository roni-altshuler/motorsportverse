...
/**
 * Rally live context band — WRC port of the RaceIQ F1 flagship's band.
 *
 * Sticky strip just below the navbar that surfaces the next round's context.
 * WRC has no live-timing feed (data reality), so this is a thinned version: it
 * shows the next upcoming rally with its surface, a countdown and a freshness
 * indicator, driven by the client-side wrc.json loader.
 *
...
import { useWrcData } from "@/lib/wrcClient";
...
export default function LiveContextBand() {
  ...
  const targetIso = nextRound.date;
    ...
    >
      <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-3 px-4 py-2 text-sm sm:px-6 lg:px-8">
        ...
        <span
          ...
          data-surface={nextRound.surface}
          style={
            nextRound.surfaceColor
              ? ({ "--surface-color": nextRound.surfaceColor } as React.CSSProperties)
              : undefined
          }
        >
          {surfaceLabel(nextRound.surface)}
        </span>
