...
// the shared design tokens like the rest of the WRC chart family.
//
// Left column: the model's pre-rally predicted order. Right column: the actual
// classified result. Each crew's line connects the two, coloured by manufacturer,
// so the reader sees at a glance where the model was on the money (flat lines)
// and where the rally broke from the script (steep lines). Crews who were
  ...
  return (
    <div>
      ...
      <svg
        ...
        role="img"
        aria-label="Slope chart connecting each crew's predicted finishing position to their actual result"
      >
        {/* Column headers */}
        <text
          x={xLeft - 8}
      ...
      <p className="mt-3 text-xs text-[var(--ink-dim)]">
        ...
        within three places. ·NC marks crews not classified in the rally.
      </p>
    </div>
  );
}
