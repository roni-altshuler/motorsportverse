...
/**
 * Win-market-by-round chart — the WRC adaptation of the F1 flagship's
 * WinProbabilityChart, reshaped around the data WRC actually has.
 *
 * F1 charts a single round's win market as bars. WRC's probability layer
 * re-forecasts every rally leakage-safely, so the richer story here is the
 * EVOLUTION: each probabilities/round_NN.json carries the win market exactly
 * as it stood before that round. This chart draws that pre-rally win
 * probability per contender across rounds, up to and including the next
 * upcoming rally (rounds beyond the next would all show the same conditioning
 * state, so they are honestly excluded).
 *
 * Values are the model's win probabilities (`rawProbability` in the market
 * files — the same quantity every other chart on the site labels
  ...
  return (
    <div className="w-full">
      ...
      <p className="mt-2 text-xs text-[var(--ink-dim)]">
        Each point is the win probability the model published before that rally
        — a forward-time record, never revised after results.
      </p>
    </div>
  );
}
