...
export async function generateMetadata({
  ...
  const title = `${fullName} — WRC ${seasonYear} Driver Profile`;
  const bits: string[] = [];
  ...
  const description =
    bits.length > 0
      ? `${fullName}: ${bits.join(", ")}. Season form, points progression, and predicted-vs-actual results for the ${seasonYear} WRC season.`
      : `${fullName}'s ${seasonYear} WRC season profile: form, points progression, and predicted-vs-actual results.`;
