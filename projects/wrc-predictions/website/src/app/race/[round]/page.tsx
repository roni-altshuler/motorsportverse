...
import { allRoundNumbers, getWrcData, getProbabilities, getRound } from "@/lib/wrcData";
 ...
 */
function buildWinTrend(data: WrcData): WinProbabilityTrend | null {
  ...
  for (let r = 1; r <= lastRound; r++) {
    ...
    if (!probs) continue;
    rounds.push(r);
    perRound.push(probs.rally.markets.win ?? {});
  }
...
export async function generateMetadata({
  ...
  if (!round) return { title: "Round — RaceIQ WRC" };
  return {
    title: `Round ${round.round}: ${round.venueName} — RaceIQ WRC`,
    description: `Rally forecast for the WRC ${round.venueName} round — win and podium probabilities and the model's full classification.`,
  };
}
