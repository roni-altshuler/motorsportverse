...
import PhaseComparisonPanel from "@/components/accuracy/PhaseComparisonPanel";
...
export const metadata: Metadata = { title: "Accuracy — RaceIQ WRC" };
...
export default function AccuracyPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-16">
      <p className="eyebrow mb-3">WRC · {data.season}</p>
      <h1 className="font-display text-4xl font-bold tracking-tight text-[var(--ink)] sm:text-5xl">
        Model accuracy
      </h1>
      <p className="mt-3 text-[var(--ink-muted)]">
        How the WRC model&rsquo;s leakage-safe pre-rally forecasts have scored against the actual
        results, over {acc?.roundsScored ?? data.completedRounds} completed rounds of {data.season}.
        Every number is scored finishers-only, using only data available before each rally.
      </p>

      <ShareButton
        title={`${data.season} WRC Prediction Accuracy`}
        text={`How well the RaceIQ model called the ${data.season} World Rally Championship — winner-hit rate, podium accuracy and per-round scoring vs honest baselines.`}
        className="mt-6"
      />
        ...
        <section className="mt-12">
          ...
          <div className="overflow-x-auto rounded-[var(--radius-lg)] border border-[var(--hairline)]">
            <table className="w-full text-sm">
              ...
              <tbody>
                {rounds.map((r) => {
                  const winBrier = r.markets?.win?.brier;
                  return (
                    <tr key={r.round} className="border-t border-[var(--hairline)] bg-[var(--surface)]">
                      ...
                      <td className="px-4 py-3">
                        <span style={{ color: r.rally.winner_hit ? "var(--accent)" : "var(--ink-dim)" }}>
                          {r.rally.winner_hit ? "✓ hit" : "miss"}
                        </span>
                      </td>
                      <td className="px-4 py-3 tabular-nums text-[var(--ink-muted)]">
                        {r.rally.podium_hits ?? 0}/3
                      </td>
                      <td className="px-4 py-3 tabular-nums text-[var(--ink-muted)]">
                        {r.rally.mean_position_error ?? "—"}
                      </td>
                      <td className="hidden px-4 py-3 tabular-nums text-[var(--ink-muted)] sm:table-cell">
                        {r.rally.ndcg_at_5 != null ? r.rally.ndcg_at_5.toFixed(2) : "—"}
                      </td>
                      <td className="hidden px-4 py-3 tabular-nums text-[var(--ink-muted)] sm:table-cell">
                        {fmt(winBrier, 4)}
                      </td>
      ...
      {/* Honest model-vs-baseline: forecast vs championship form + last-rally */}
      <PhaseComparisonPanel season={season} />

      {/* Walk-forward: model vs the championship-form baseline */}
