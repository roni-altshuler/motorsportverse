/**
 * HomePage — RaceIQ Formula E home, ported from the RaceIQ F1 flagship.
 *
 * Server component: reads the build-time FE dataset with `getFEData()` and
 ...
 *
 * Adapted from F1: Formula E runs ONE scored race per round (no sprint), has no
 * telemetry / weather / pit strategy feeds, and no separate trust-stats JSON.
 * Trust numbers are derived from the season accuracy block. Doubleheaders are
 * separate rounds sharing a venue, and the championship is split-year
 * ("2025-26") — the season label comes from the export.
 */
import Link from "next/link";

import { getCircuit, getFEData, getRound } from "@/lib/fedata";
import { teamColor } from "@/lib/teams";
import type { RaceBlock } from "@/types/fe";
...
export default function HomePage() {
  ...
  // Split-year championship label, e.g. "2025-26" (fe.json ships
  // "SEASON 2025-2026"; fall back to the season end-year).
  const seasonLabel = `${data.season - 1}-${String(data.season).slice(2)}`;

  // The predicted podium only teases when a next-round forecast exists — FE's
  ...
  return (
    <div>
      <HeroParallax className="min-h-[78vh] flex items-center" geometry={nextGeometry}>
        <div className="mx-auto w-full max-w-6xl px-6 lg:px-10 py-20">
          ...
          <div className="max-w-3xl">
            <p className="eyebrow mb-5">RaceIQ · Formula E · Season {seasonLabel}</p>
            <h1 className="display-xl [font-weight:700] text-balance">
              Formula E, forecast.
            </h1>
            <p className="body-md mt-6 max-w-2xl text-[color:var(--body-strong)]">
              Race and championship forecasts for the ABB FIA Formula E World
              Championship — every E-Prix, from the walled street circuits to the
              permanent tracks, including both races of every doubleheader
              weekend. {data.completedRounds} of {data.totalRounds} rounds
              complete, on the same MotorsportVerse core that powers RaceIQ F1.
            </p>
          </div>
            ...
            <div className="mt-12 border-t border-[color:var(--hairline)] pt-8">
              <div className="flex flex-wrap items-center gap-4 mb-6">
                ...
                <span className="eyebrow">
                  R{next.round} ·{" "}
                  {nextCalendarRound.kind === "street" ? "Street Circuit" : "Permanent Circuit"}
              ...
              <div className="mb-8">
                ...
                <p className="body-md mt-3 max-w-2xl text-[color:var(--muted)]">
                  {nextCalendarRound.country ?? "Round " + next.round} · one scored
                  race — a full-field forecast before the grid forms.
                </p>
              </div>
      ...
      <div className="mx-auto max-w-6xl px-6 lg:px-10">
            ...
            <div className="flex items-baseline justify-between mb-12">
              <div>
                ...
                <p className="body-md mt-4 max-w-2xl text-[color:var(--muted)]">
                  The model&apos;s top three picks for the {next.venueName} E-Prix
                  at round {next.round}. Projected race winner plus the two
                  drivers most likely to share the rostrum.
                </p>
              </div>
            ...
            <div className="flex items-baseline justify-between mb-8">
              <div>
                ...
                <p className="body-md mt-3 text-[color:var(--muted)]">
                  Round {latestCompleted.round} · {latestCompleted.name} E-Prix
        ...
        {/* ── Teams constellation ── Formula E's manufacturers each bring their
            own powertrain, so the teams' championship is a real title fight. */}
          ...
          >
            <div className="text-center mb-10">
              ...
              <p className="body-md mt-3 max-w-xl mx-auto text-[color:var(--muted)]">
                {data.teamStandings.length} teams contest the {seasonLabel} Formula E
                season — every car on the grid, in orbit. Each manufacturer brings
                its own powertrain, so the teams&apos; title is a genuine
                engineering fight.
              </p>
            </div>
