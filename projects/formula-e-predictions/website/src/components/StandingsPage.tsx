...
// season's fe.json (client-side). Uses each row's exported pointsHistory —
// the same fallback the server page's series builder uses.
function propsFromSeasonData(data: FEData): StandingsPageProps {
  const TOP_DRIVERS = 6;
  const TOP_TEAMS = 5;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 section-bugatti">
      <div className="mb-16 text-center">
        <p className="eyebrow mb-4">Formula E · Championship</p>
        <h1 className="display-xl mb-4">{season - 1}-{String(season).slice(2)} Standings</h1>
        ...
        <div className="mt-6 flex justify-center">
          <ShareButton
            title={`Formula E ${season - 1}-${String(season).slice(2)} — championship standings`}
            text="Formula E championship standings, projections, and who can still win — from the RaceIQ model."
          />
        </div>
      </div>
              ...
              return (
                <tr key={d.code} style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  <td className="px-4 py-3">
                    <Link
                      href={`/driver/${d.code}`}
                      className="group inline-flex items-baseline hover:text-[color:var(--accent-f1-red-bright)] transition-colors"
                    >
                      <span className="font-bold text-[color:var(--ink)] group-hover:text-[color:var(--accent-f1-red-bright)]">
                        {d.code}
                      </span>
                      <span className="ml-2 text-xs hidden sm:inline text-[color:var(--text-muted)] group-hover:text-[color:var(--body-strong)]">
                        {d.name}
                      </span>
                    </Link>
                  </td>
