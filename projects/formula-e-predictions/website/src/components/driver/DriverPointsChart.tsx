...
/**
 * DriverPointsChart — cumulative championship-points progression for one driver.
 *
 * Sourced from each completed round's race result via `driverPointsProgression`
 * (reconstructed then scaled to the driver's standings total). Team-coloured
 * area line; the tooltip surfaces both the running total and the points scored
 * in that round. Static, non-interactive beyond hover — no animation loop, so
 * it stays cheap on a static export. Ported from the RaceIQ F1 flagship.
 */
import {
  Area,
  AreaChart,
...
function PointsTooltip({ active, payload }: TooltipProps) {
  ...
  const delta = Math.round(p.delta);
  return (
    <div className="rounded-none border border-[color:var(--hairline)] bg-[color:var(--surface-card)] px-3 py-2 text-xs shadow-lg">
      ...
      <div className="font-mono tabular-nums text-[color:var(--ink)]">
        {fmt(p.cumulative)} pts total
      </div>
      <div className="font-mono tabular-nums text-[color:var(--muted)]">
...
export default function DriverPointsChart({
  ...
  const gradientId = "fe-driver-points-fill";
