...
import type { ModelHealth } from "@/types/fe";

/**
 * ModelHealthPanel — a small, honest "is the model still healthy?" strip
 * (ported from the RaceIQ F1 flagship, adapted to Formula E's model_health.json
 * shape + electric-blue identity).
 *
 * It separates the two things that are easy to conflate: whether the FORECASTS
 * are still landing well (output quality, the rolling probability-error trend)
 * versus whether the model's INPUTS have drifted from what it was tuned on.
 * Input drift is watched but does not, on its own, mean the predictions have
 * degraded — so the headline status follows output quality and input shifts are
 * shown as a separate signal.
 *
 * Every field is optional; the panel renders only what the file provides and
 * hides entirely when the file is missing.
 */

const FEATURE_LABELS: Record<string, string> = {
  predictedValue: "Predicted merit",
...
// FE identity: electric blue is the site's "negative" signal (--accent-negative),
// so a shifted/degraded state reads blue rather than the F1 red.
const TONE_DOT: Record<Tone, string> = {
  good: "var(--success)",
  warn: "var(--warning)",
  bad: "var(--accent-negative)",
...
const TONE_PILL: Record<Tone, { background: string; color: string }> = {
  ...
  bad: { background: "rgba(30,26,240,0.16)", color: "var(--accent-f1-red-bright)" },
  ...
  return (
    <section className="mt-12">
      ...
      <p className="text-sm mb-5 max-w-2xl" style={{ color: "var(--text-muted)" }}>
        A self-check on the live model: whether recent forecasts are still landing as well as
        they should, and whether the numbers feeding the model have drifted from what it was
        tuned on.
      </p>
        ...
        <div className="mb-4">
          <div className="mb-3">
            ...
            <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
              How far the model&apos;s probabilities sat from what actually happened — lower is a
              ...
              <LineChart data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                <Tooltip
                  ...
                  formatter={(value) => {
                    ...
                    return [Number(value).toFixed(3), "Avg error"];
                  }}
                />
      ...
      {shiftedCount > 0 && (
        <p className="text-xs leading-relaxed" style={{ color: "var(--text-muted)" }}>
          Some model inputs have drifted from their reference range — normal across a season and
          with a small sample of races. It is flagged here for transparency, but the rolling
          forecast-quality check above shows the predictions themselves are{" "}
