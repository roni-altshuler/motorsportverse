import type { CalibrationSummary } from "@/types/fe";
...
/**
 * Renders calibration_summary.json — the honest calibration status the Formula
 * E model earned on real rounds. Adapts the F1 flagship's calibration panel to
 * FE's shape: applied flag, training-round count, the street/circuit strata the
 * calibrator was fitted on, and the per-market observation counts. (The values
 * in `perMarket` are sample counts, not temperatures — labelled as such.)
 */
    ...
    <section className="mt-12">
      ...
      <div className="rounded-[var(--radius-lg)] border border-[var(--hairline)] bg-[var(--surface)] p-6">
        <p className="text-sm text-[var(--ink-muted)]">
          ...
          classified results — separately for street and permanent circuits, because
          the two race very differently — so a stated 30% podium chance means roughly
          3-in-10 over the long run.
        </p>
            ...
            {summary.strata && Object.keys(summary.strata).length > 0 && (
              <p className="mt-1 mb-1 flex flex-wrap gap-1.5">
                {Object.keys(summary.strata).map((k) => (
                  ...
                  >
                    {k === "street" ? "Street stratum" : k === "circuit" ? "Circuit stratum" : k}
                  </span>
                ))}
              </p>
            )}
