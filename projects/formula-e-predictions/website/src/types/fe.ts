// TypeScript mirror of the JSON produced by formula_e_predictions.export + the
...
//
// Formula E weekends run ONE scored race per round (no sprint/feature split);
// doubleheaders are separate rounds sharing a venue `key` ("Jeddah"/"Jeddah II"),
// and every round carries a `venueKind` (street vs permanent circuit).

/** Street E-Prix vs permanent circuit — carried on calendar + round payloads. */
export type VenueKind = "street" | "circuit";

// --------------------------------------------------------------------------- //
// Season summary — public/data/fe.json
// --------------------------------------------------------------------------- //
...
export interface FEData {
  sport: string;
  season: number;
  /** Split-year championship label, e.g. "SEASON 2025-2026". */
  seasonLabel?: string;
  generatedAt?: string;
...
/** A/B lever provenance for the finishing-position head (FE_USE_POSITION_HEAD,
...
export interface CalibrationSummary {
  ...
  /** Calibration strata → markets fitted (FE stratifies street vs circuit). */
...
export interface ForwardEvalSeason {
  ...
  winnerHitRate: number | null;
  podiumHitRate: number | null;
  /** Walk-forward headline block (F1 parity); keyed "race" for Formula E. */
  generatedAt?: string;
...
export interface HistoricalBacktest {
  ...
  totalRows: number;
  /** All-seasons pooled summary (additive; FE ships it, older exports may not). */
