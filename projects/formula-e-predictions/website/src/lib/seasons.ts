 ...
 * index derived from fe.json when the site predates multi-season support,
...
export async function fetchSeasonsIndex(): Promise<SeasonsIndex> {
  ...
  // Legacy fallback: derive the current season from fe.json.
  let current = new Date().getFullYear();
  try {
    const res = await fetch(`${BASE_PATH}/fe.json`);
    if (res.ok) {
