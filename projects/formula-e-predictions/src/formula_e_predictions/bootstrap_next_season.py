...
FE calendars are announced mid-year for the season starting in December; until
the official calendar lands this module writes a *placeholder* announced
calendar: the active season's venues carried forward with the race dates
shifted by +364 days per year (same weekday), plus the current teams and
roster flagged ``lineup_provisional``. The payload lands in
...
live refresh then self-corrects the roster once the season's first entry list
is published (pin the new championship id in ``config.CHAMPIONSHIP_IDS``).
...
Usage:
    python -m formula_e_predictions.bootstrap_next_season               # active year + 1
    python -m formula_e_predictions.bootstrap_next_season --year 2027 --force
    python -m formula_e_predictions.bootstrap_next_season --from-file announced_2027.json
    python -m formula_e_predictions.bootstrap_next_season --dry-run
"""
        ...
        calendar.append(
            {
                ...
                "kind": str(getattr(venue.kind, "value", venue.kind)),
                "city": meta.get("city", ""),
...
def bootstrap(
    ...
    *,
    ...
    print(
        f"Bootstrapping season {year} / {config.season_label(year)} ({kind}): "
...
def main() -> None:
    ...
    ap.add_argument("--year", type=int, default=None,
                    help="Season to bootstrap, keyed by ENDING year "
                         "(default: active season + 1).")
