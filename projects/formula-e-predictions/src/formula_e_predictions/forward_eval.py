"""Forward-time evaluation for FE — scores leakage-safe predictions vs actuals.

**This is the headline validation surface for the FE model.** For every
...
:func:`motorsport_core.eval.score_round` bundle. Scoring is **finishers-only**:
retirements are unranked in the classification, so every metric compares the
prediction to drivers who took the flag — the family convention.
...
* ``gridOrder`` — the round's actual starting grid as the prediction (a real
  pre-race-known baseline; on FE street circuits it is a *strong* one, which
  is exactly why it must be beaten, not ignored).
...
Run:  python -m formula_e_predictions.forward_eval --season 2026 [--allow-empty]
...
def _actuals(source: FEDataSource, year: int, rnd: int) -> dict[str, int]:
    return {
...
def _grid_map(source: FEDataSource, year: int, rnd: int) -> dict[str, int] | None:
    """The round's actual starting grid ({code: slot}), or None when unknown.

    Prefers the classification rows' recorded grid; falls back to the captured
    qualifying order. Real sources only — a synthetic grid would not be a
    meaningful baseline.
    """
...
def evaluate_season(year: int) -> list[dict]:
    ...
    Each per-round dict carries ``round``/``venueName``/``venueKind``/``race``
...
def build_walk_forward_summary(rounds: list[dict]) -> dict:
    ...
    block — the season-level headline surface."""
...
def main() -> int:
    ...
    p.add_argument("--min-prior-rounds", type=int, default=None,
                   help="Minimum prior completed rounds before the position head "
                        "trains (default 3; below this a round is recorded "
    ...
    try:
        evidence_path = write_evidence_artifact(args.out, "formula-e")
