"""Direct finishing-position head for FE (opt-in alternative race-order head).

FE port of the F1 flagship's ``models/position_model.py`` (A/B-gated,
verdict-driven; F3 carries the same port). The production FE race order is a
*post-processing* of the latent-skill blend: :mod:`.model` sorts drivers by
simulated mean finish from a pace score — it never learns finishing position
directly. This module learns finishing position **directly**, leakage-safe, so
it can be A/B'd against the production path before anyone trusts it.

Gate
----
The head is wired behind the ``FE_USE_POSITION_HEAD`` environment flag
...
re-ranking on top of the production signal. FE rounds carry a single race, so
the frame has one row per (round, classified finisher); the ``gridPosition``
and ``isStreet`` features carry FE's track-position and street-variance
structure.
...
ENV_FLAG = "FE_USE_POSITION_HEAD"
RANDOM_STATE = 42

# FE rounds contribute ~20 finisher-rows each (one race, 20-car grid), so the
# gate matches F1's three prior rounds (~60 rows) rather than F3's two.
...
FEATURE_NAMES: tuple[str, ...] = (
    ...
    "gridPosition",      # starting slot (1 = pole) — track position on street circuits
    "finishRangeWidth",  # rangeHigh - rangeLow (MC uncertainty span)
    "finishRangeLow",    # optimistic MC finish bound
    "isStreet",          # 1.0 for a street circuit, 0.0 for a permanent one
)
...
def head_enabled(override: bool | None = None) -> bool:
    """A/B gate: explicit override wins, else the ``FE_USE_POSITION_HEAD`` env
...
def extract_race_features(
    race_fc: "RaceForecast", *, venue_kind: str = "street"
    ...
    comparable across venues without leaking absolute pace levels; the one
    venue-level bit is the street/circuit flag.
    """
    ...
    is_street = 1.0 if str(venue_kind) == "street" else 0.0
...
def build_training_frame(
    forecasts_by_round: Mapping[int, "RoundForecastFE"],
    ...
    ``actual_by_round`` maps round → ``{driver: position}`` (classified
    finishers only, so the frame is finishers-only like the headline eval).
    ...
    assert_prior_only(
        prior_map, current_round=int(target_round), label="fe.position_head.training_rounds"
    )
    ...
    for rnd in sorted(prior_map):
        ...
        if not actual_race:
            continue
        fc = prior_map[rnd]
        feats = extract_race_features(fc.race, venue_kind=fc.venue_kind)
        contributed = False
...
class PositionHead:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "PositionHead":
        ...
        # the frame is small (a handful of rounds × ~20 rows).
        try:
...
# --------------------------------------------------------------------------- #
# Wiring against the FE data/model layer
# --------------------------------------------------------------------------- #
def _actuals_for_round(source: "FEDataSource", year: int, rnd: int) -> dict[str, int]:
    return {
...
def _prior_replays(
    source: "FEDataSource", year: int, target_round: int
) -> tuple[dict[int, "RoundForecastFE"], dict[int, dict[str, int]]]:
    ...
    *production* signal — never the head's own output (no feedback loop).
    ...
    forecasts: dict[int, "RoundForecastFE"] = {}
...
def train_for_round(
    ...
    *,
    ...
    forecasts_by_round: Mapping[int, "RoundForecastFE"] | None = None,
...
def rerank_race(
    head: PositionHead, race_fc: "RaceForecast", *, n_samples: int, venue_kind: str = "street"
) -> "RaceForecast":
    """Re-run the race's Monte Carlo with the head's predicted positions as the
    score.
    ...
    feats = extract_race_features(race_fc, venue_kind=venue_kind)
...
def maybe_rerank_round(
    ...
    *,
    ...
    graceful degradation contract as the other optional FE signals.
    """
    ...
    race = rerank_race(head, fc.race, n_samples=n_samples, venue_kind=fc.venue_kind)
    return replace(
        fc,
        race=race,
        position_head={
...
def run_backtest(
    ...
    *,
    ...
    against the actual classified finishers via
