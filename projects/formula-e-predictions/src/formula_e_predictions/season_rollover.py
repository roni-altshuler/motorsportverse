"""Season rollover — archive a completed FE season and start the next one.

Ported from the F3 golden template (itself from the F1 flagship) and adapted to
FE's conventions: seasons are keyed by ENDING year (the 2025-26 championship is
``2026``), the season lives in ``config.py`` (2026 literals + announced-calendar
overrides), results come from the committed snapshot ``data/official_<year>.json``,
and the website data contract is produced by ``formula_e_predictions.export``
into ``website/public/data/``.
  ...
  --auto             If the active season is COMPLETE (the snapshot says every
                     ...
                     refreshes seasons.json — safe to call from the FE cron on
                     every run.

All steps support --dry-run.

Run:  python -m formula_e_predictions.season_rollover --auto [--dry-run]
"""
    ...
    *,
    ...
    if year != config._DEFAULT_SEASON and _load_announced(year, announced_dir) is None:
        raise SystemExit(
            ...
            f"`python -m formula_e_predictions.bootstrap_next_season --year {year}` first."
        )
    ...
    print(
        f"Season {year} is now active. Run `python -m formula_e_predictions.refresh` and "
        "`python -m formula_e_predictions.export` to populate it."
    )
    ...
    *,
    ...
    if announced is None:
        return _no_op(
            ...
            f"— run `python -m formula_e_predictions.bootstrap_next_season`."
        )
