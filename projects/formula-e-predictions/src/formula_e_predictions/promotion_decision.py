"""Promotion gate for FE — thin CLI over motorsport_core.promotion.
...
A/B-gated by ``FE_USE_POSITION_HEAD`` and OFF by default). Its walk-forward
evidence is the ``forward_eval/position_model_ab.json`` artifact written by
``python -m formula_e_predictions.forward_eval --position-model-ab``: per
...
Run:  python -m formula_e_predictions.promotion_decision --season 2026
          [--allow-empty]
"""
...
def build_status(data_dir: Path) -> dict:
    ...
    return {
        ...
        "candidateFlag": "FE_USE_POSITION_HEAD",
