...
// MotoGP has no fs-free lib/data.ts, so the data root constant lives here (safe to
 ...
 * index derived from motogp.json when the site predates multi-season support,
...
export async function fetchSeasonsIndex(): Promise<SeasonsIndex> {
  ...
  // Legacy fallback: derive the current season from motogp.json.
  let current = new Date().getFullYear();
  try {
    const res = await fetch(`${BASE_PATH}/motogp.json`);
    if (res.ok) {
