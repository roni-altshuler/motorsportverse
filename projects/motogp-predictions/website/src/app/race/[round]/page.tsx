...
import { allRoundNumbers, getMotogpData, getProbabilities, getRound } from "@/lib/motogpData";
 ...
 */
function buildWinTrend(data: MotogpData): WinProbabilityTrend | null {
  ...
  for (let r = 1; r <= lastRound; r++) {
    ...
    if (!probs) continue;
    rounds.push(r);
    perRound.push(probs.feature.markets.win ?? {});
  }
...
export async function generateMetadata({
  ...
  if (!round) return { title: "Round — RaceIQ MotoGP" };
  return {
    title: `Round ${round.round}: ${round.venueName} — RaceIQ MotoGP`,
    description: `Sprint and Grand Prix forecasts for the MotoGP ${round.venueName} round.`,
  };
}
