...
export const metadata: Metadata = { title: "Accuracy — RaceIQ MotoGP" };
...
export default function AccuracyPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-16">
      <p className="eyebrow mb-3">MotoGP · {data.season}</p>
      <h1 className="font-display text-4xl font-bold tracking-tight text-[var(--ink)] sm:text-5xl">
        Model accuracy
      </h1>
      <p className="mt-3 text-[var(--ink-muted)]">
        How the MotoGP model&rsquo;s leakage-safe pre-race forecasts have scored against the actual
      ...
      <ShareButton
        title={`${data.season} MotoGP Prediction Accuracy`}
        text={`How well the RaceIQ model called the ${data.season} MotoGP season — winner-hit rate, podium accuracy and per-round scoring vs honest baselines.`}
        className="mt-6"
      />
      ...
      {rounds.length > 0 && (
        <section className="mt-12">
          <h2 className="mb-4 text-xl font-semibold text-[var(--ink)]">Per round (Grand Prix)</h2>
      ...
      {/* Honest model-vs-baseline: grid-conditioned forecast vs the grid + form-only */}
