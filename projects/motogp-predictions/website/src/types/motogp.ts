// TypeScript mirror of the JSON produced by motogp_predictions.export + the
// forward_eval / drift_report CLIs.
//
...
// --------------------------------------------------------------------------- //
// Season summary — public/data/motogp.json
// --------------------------------------------------------------------------- //
export interface CalendarRound {
  ...
  country: string | null;
  city?: string;
  /** Official event name, e.g. "Michelin Grand Prix of Aragon". */
  event?: string;
  /** ISO date of the Sunday Grand Prix (the single date the export emits). */
  date?: string;
  /** Optional explicit session dates; MotoGP derives the Saturday sprint from `date`. */
  sprintDate?: string;
  featureDate?: string;
  completed: boolean;
...
/** A/B lever provenance for the finishing-position head (MOTOGP_USE_POSITION_HEAD,
...
/** Post-quali vs pre-quali vs grid-baseline scores for one probability market. */
export interface PhaseComparisonMetric {
  post: number;
  pre: number;
  grid: number;
}

/** Honest model-vs-baseline comparison (grid-conditioned forecast vs the grid). */
export interface PhaseComparison {
  note: string;
  roundsScored: number;
  /** market name → {post, pre, grid} (winBrier / podiumBrier / winnerHit). */
  feature: Record<string, PhaseComparisonMetric>;
  beatsGridBaseline: boolean;
}
...
export interface ForwardEvalSeason {
  ...
  /** Additive: post-quali model vs pre-quali form-only vs grid baseline. */
