...
import { useSeasonMotogpData } from "@/lib/motogpClient";
...
// season's motogp.json (client-side). Uses each row's exported pointsHistory —
// the same fallback the server page's series builder uses.
function propsFromSeasonData(data: MotogpData): StandingsPageProps {
  const TOP_DRIVERS = 6;
  const TOP_TEAMS = 5;
...
function StandingsPageInner(baked: StandingsPageProps) {
  ...
  const { data: seasonData, isArchived } = useSeasonMotogpData();
  const {
    season,
    completedRounds,
    totalRounds,
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 section-bugatti">
      <div className="mb-16 text-center">
        <p className="eyebrow mb-4">MotoGP · Championship</p>
        ...
        <ShareButton
          title={`${season} MotoGP Championship Standings`}
          text={`See who's winning the ${season} MotoGP title — full rider & manufacturer standings, plus who can still take it.`}
      ...
      {activeTab === "teams" && (
        <div className="space-y-8">
          ...
          {rounds.length > 0 && teamSeries.length > 0 && (
            <div className="card p-6">
              <h3 className="section-heading">Manufacturer Points Progression</h3>
              <ProgressionChart
                series={teamSeries}
                rounds={rounds}
                totalRounds={totalRounds}
          ...
          <div className="space-y-3">
            <h3 className="section-heading">Manufacturers — Still in the Fight</h3>
      ...
      {activeTab === "wdc" && (
        <div className="space-y-10">
          {rounds.length > 0 && driverSeries.length > 0 && (
            <div className="card p-6">
              <h3 className="section-heading">Riders — Points Projection</h3>
              <ProgressionChart
                series={driverSeries}
                rounds={rounds}
                totalRounds={totalRounds}
          ...
          <div className="space-y-6">
            <div>
              <h2 className="display-md mb-2">Manufacturers Title Race</h2>
              <p className="body-md text-[color:var(--text-muted)] max-w-2xl">
                Manufacturer points standings and who can still mathematically reach the leader over the{" "}
                {remainingRounds} remaining round{remainingRounds === 1 ? "" : "s"}.
              </p>
            </div>
            {rounds.length > 0 && teamSeries.length > 0 && (
              <div className="card p-6">
                <h3 className="section-heading">Manufacturers — Points Projection</h3>
                <ProgressionChart
                  series={teamSeries}
                  rounds={rounds}
                  totalRounds={totalRounds}
  ...
  return (
    <div className="card overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-sm" aria-label="Rider championship standings">
          <thead>
              ...
              return (
                <tr key={d.code} style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  <td className="px-2 py-2">
                    <Link
                      href={`/rider/${d.code}`}
                      aria-label={`${d.name} rider profile`}
