 ...
 *
 * Sourced from the standings row's `pointsHistory` (cumulative points per round)
 * via `pointsProgression()`. Team-coloured area line; the tooltip surfaces both
 * the running total and the points scored in that round. Static beyond hover —
 * no animation loop, so it stays cheap on a static export. Ported from the
 * RaceIQ F1 flagship's driver chart, wired to MotoGP's `PointsProgressionPoint`.
 */
import {
  Area,
  AreaChart,
