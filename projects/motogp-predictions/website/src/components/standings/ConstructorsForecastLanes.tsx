...
import type { TeamStanding } from "@/types/motogp";
 ...
 *
 * MotoGP ships NO constructor title-odds array, so "who can still win" is DERIVED
  ...
  if (!teams || teams.length === 0) {
    return (
      <div className="card p-8 text-center">
        <p className="eyebrow mb-2">Manufacturers outlook not yet available</p>
        <p className="body-sm text-[color:var(--text-muted)] max-w-md mx-auto">
          The manufacturers&apos; title outlook publishes after the first round completes.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="card p-4 sm:p-6">
      ...
      {eliminated.length > 0 && (
        <details className="mt-6 group">
          <summary className="cursor-pointer eyebrow text-[color:var(--text-muted)] hover:text-[color:var(--text)] transition-colors">
            Mathematically out · {eliminated.length} manufacturer
      ...
      <p className="mt-4 text-xs text-[color:var(--text-muted)]">
        &ldquo;Still in the fight&rdquo; is derived from the points standings: a manufacturer is shown as
        alive while its maximum attainable total over the remaining rounds still reaches the
        leader. Projected totals extrapolate each manufacturer&apos;s current pace.
      </p>
    </div>
  );
}
