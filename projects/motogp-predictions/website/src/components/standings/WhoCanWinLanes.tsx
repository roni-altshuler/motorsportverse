...
import type { TitleOdds } from "@/types/motogp";
 ...
 * Sourced directly from MotoGP's `championship[]` (TitleOdds). Drivers below the
  ...
  return (
    <div className="space-y-6">
        ...
        {eliminated.length > 0 && (
          <details className="mt-6 group">
            <summary className="cursor-pointer eyebrow text-[color:var(--text-muted)] hover:text-[color:var(--text)] transition-colors">
              Mathematically out · {eliminated.length} rider
      ...
      <p className="text-xs text-[color:var(--text-muted)]">
        Title odds are projected from each rider&apos;s current form across the remaining
        weekends, applying the MotoGP points system (Grand Prix + sprint) and accounting
        for retirement risk.
      </p>
    </div>
  );
