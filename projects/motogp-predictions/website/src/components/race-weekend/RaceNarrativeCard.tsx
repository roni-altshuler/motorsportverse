...
/**
 * Race-weekend narrative card — MotoGP adaptation of the F1 flagship's
 ...
 *
 * The rules are rebuilt around the data MotoGP actually has (there is no weather
 ...
 *   3. Championship stakes — leader's title probability and how many riders
...
import type { RoundDetail, TitleOdds } from "@/types/motogp";
...
function buildBullets(
  ...
  const raceLabel = race === "feature" ? "Grand Prix" : "sprint";
  ...
  // 2. Sprint context: MotoGP's Saturday Sprint starts from the SAME qualifying
  //    grid as the Grand Prix but runs about half the distance, so track position
  //    holds up more and the race is higher-variance. Surface the front-row start.
  if (race === "sprint" && block.grid.length > 0) {
    const poleCode = block.grid[0].code;
    const forecast = cls.find((e) => e.code === poleCode);
    if (forecast) {
      out.push({
        label: "SPRINT",
        tone: "info",
        text: `Shorter, higher-variance Sprint off the same grid: pole-starter ${forecast.name} is projected around P${Math.round(forecast.meanFinish)} (range P${forecast.finishRangeLow}–P${forecast.finishRangeHigh}).`,
      });
    }
  }
  ...
  if (!round.completed && championship.length > 0) {
    ...
    if (leader.pTitle >= TITLE_GRIP_P) {
      out.push({
        label: "TITLE",
        tone: "positive",
        text: `${leader.name} has one hand on the title (${(leader.pTitle * 100).toFixed(0)}% on ${leader.currentPoints} pts); ${alive} riders remain mathematically alive.`,
      });
      ...
      out.push({
        label: "TITLE",
        tone: "default",
        text: `Title fight: ${leader.name} (${(leader.pTitle * 100).toFixed(0)}%) vs ${rival.name} (${(rival.pTitle * 100).toFixed(0)}%), ${alive} riders still mathematically in it.`,
      });
    }
  }
