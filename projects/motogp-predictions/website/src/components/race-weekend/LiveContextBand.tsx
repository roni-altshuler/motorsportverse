...
/**
 * Race-weekend live context band — MotoGP port of the RaceIQ F1 flagship's band.
 *
 * Sticky strip just below the navbar that surfaces the next round's context.
 * MotoGP has no live-timing or weather feed (data reality), so this is a thinned
 * version: it shows the next upcoming round (sprint + feature) with a countdown
 * and a freshness indicator, driven by the client-side motogp.json loader.
 *
...
import { useMotogpData } from "@/lib/motogpClient";
...
export default function LiveContextBand() {
  ...
  const targetIso = nextRound.featureDate ?? nextRound.sprintDate;
