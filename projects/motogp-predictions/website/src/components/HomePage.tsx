/**
 * HomePage — RaceIQ MotoGP home, ported from the RaceIQ F1 flagship.
 *
 * Server component: reads the build-time MotoGP dataset with `getMotogpData()` and
 ...
 *
 * Adapted from F1: MotoGP publishes one Sunday Grand Prix date per round (the
 * Saturday sprint is derived from it), no telemetry / weather / pit strategy,
 * and no separate trust-stats JSON. Trust numbers are derived from the season
 * accuracy block. MotoGP runs both a rider and a manufacturer world championship
 * (Ducati, Aprilia, KTM, Yamaha, Honda), so the constellation is the
 * manufacturers behind the grid.
 */
import Link from "next/link";

import { getCircuit, getMotogpData, getRound } from "@/lib/motogpData";
import { teamColor } from "@/lib/teams";
import type { RaceBlock } from "@/types/motogp";
...
export default function HomePage() {
  ...
  // The predicted podium only teases when a next-round forecast exists — MotoGP's
  ...
  return (
    <div>
      <HeroParallax className="min-h-[78vh] flex items-center" geometry={nextGeometry}>
        <div className="mx-auto w-full max-w-6xl px-6 lg:px-10 py-20">
          ...
          <div className="max-w-3xl">
            <p className="eyebrow mb-5">RaceIQ · MotoGP · {data.season}</p>
            <h1 className="display-xl [font-weight:700] text-balance">
              MotoGP, forecast.
            </h1>
            <p className="body-md mt-6 max-w-2xl text-[color:var(--body-strong)]">
              Sprint, Grand Prix and championship forecasts for the premier class
              of motorcycle Grand Prix racing — a win and podium probability for
              every rider, and the rider- and manufacturer-title fights projected
              to the flag. Each round runs a Saturday sprint and a Sunday Grand
              Prix off the same qualifying grid. {data.completedRounds} of{" "}
            ...
            <div className="mt-12 border-t border-[color:var(--hairline)] pt-8">
              <div className="flex flex-wrap items-center gap-4 mb-6">
                ...
                <span className="eyebrow">
                  R{next.round} · Sprint + Grand Prix
                  {nextCalendarRound.featureDate ?? nextCalendarRound.date ? (
                    <>
                      {" · "}
                      <HeroCountdown
                        targetDate={(nextCalendarRound.featureDate ?? nextCalendarRound.date)!}
                      />
                    </>
                  ) : null}
                </span>
              ...
              <div className="mb-8">
                ...
                <p className="body-md mt-3 max-w-2xl text-[color:var(--muted)]">
                  ...
                  modelled separately — Saturday sprint and Sunday Grand Prix.
                </p>
              </div>
      ...
      <div className="mx-auto max-w-6xl px-6 lg:px-10">
            ...
            <div className="flex items-baseline justify-between mb-12">
              <div>
                ...
                <p className="body-md mt-4 max-w-2xl text-[color:var(--muted)]">
                  The model&apos;s top three picks for the Grand Prix at round{" "}
                  {next.round}. Projected race winner plus the two riders most
            ...
            <div className="flex items-baseline justify-between mb-8">
              <div>
                ...
                <p className="body-md mt-3 text-[color:var(--muted)]">
                  Round {latestCompleted.round} · {latestCompleted.name} Grand Prix
        ...
        {/* ── Manufacturers constellation ── MotoGP runs a manufacturer world
            championship alongside the riders' title, so the copy is about the
            marques that build the bikes. */}
          ...
          >
            <div className="text-center mb-10">
              ...
              <h2 id="constellation-heading" className="display-md">
                The manufacturers behind the grid
              </h2>
              <p className="body-md mt-3 max-w-xl mx-auto text-[color:var(--muted)]">
                {data.teamStandings.length} manufacturers build the {data.season}{" "}
                MotoGP field — Ducati, Aprilia, KTM, Yamaha and Honda, each chasing
                its own world title alongside the riders.
              </p>
            </div>
