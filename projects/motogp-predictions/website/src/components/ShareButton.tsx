...
/**
 * ShareButton — RaceIQ MotoGP
 *
 * Lightweight share cluster ported from the RaceIQ F1 flagship. Three
 * affordances, each with one job:
 ...
 *
 * It styles entirely through CSS custom properties (`--accent-f1-red` resolves
 * to MotoGP's championship gold under the MotoGP tokens), so it reads as part of one
 * product family. The button only hands over the current page URL (read lazily
 * from `window.location` so it stays correct under static export / base paths).
 */
import { useCallback, useState, useSyncExternalStore } from "react";

interface ShareButtonProps {
  /** Headline used by the native sheet + tweet text, e.g. the round name. */
  title: string;
...
export default function ShareButton({ title, text, className }: ShareButtonProps) {
  ...
  const shareText = text ?? `${title} — MotoGP race prediction & championship model`;
