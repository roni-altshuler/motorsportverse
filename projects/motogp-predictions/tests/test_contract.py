...
from motogp_predictions.sources import snapshot
...
def test_datasource_implements_the_core_contract():
    assert issubclass(MotoGPDataSource, CoreDataSource)
...
def test_fit_signature_matches_the_contract():
    ...
    assert list(inspect.signature(MotoGPPredictor.fit).parameters) == [
...
def test_no_snapshot_means_an_empty_calendar_not_an_invented_one():
    assert list(MotoGPDataSource().calendar(config.DEFAULT_SEASON)) == []


def test_no_snapshot_means_an_empty_grid():
    assert list(MotoGPDataSource().grid(config.DEFAULT_SEASON, 1)) == []


def test_no_snapshot_means_no_results():
    assert dict(MotoGPDataSource().results(config.DEFAULT_SEASON, 1)) == {}


def test_an_empty_grid_produces_an_empty_order_not_a_guess():
    forecast = MotoGPPredictor().predict(MotoGPDataSource(), config.DEFAULT_SEASON, 1)
...
def test_strict_mode_fails_loudly_instead_of_returning_empty():
    ...
    with pytest.raises(snapshot.SnapshotUnavailable) as excinfo:
        MotoGPDataSource(strict=True).calendar(config.DEFAULT_SEASON)
...
def test_fit_only_sees_prior_rounds():
    ...
    predictor.fit(MotoGPDataSource(), config.DEFAULT_SEASON, upto_round=5)
