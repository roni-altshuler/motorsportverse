"""Contract tests for 24 Hours of Le Mans.
...
def test_fit_signature_matches_the_contract():
    ...
    assert list(inspect.signature(LeMansPredictor.fit).parameters) == [
...
def test_no_snapshot_means_an_empty_calendar_not_an_invented_one():
    assert list(LeMansDataSource().calendar(config.DEFAULT_SEASON)) == []


def test_no_snapshot_means_an_empty_grid():
    assert list(LeMansDataSource().grid(config.DEFAULT_SEASON, 1)) == []


def test_no_snapshot_means_no_results():
    assert dict(LeMansDataSource().results(config.DEFAULT_SEASON, 1)) == {}


def test_an_empty_grid_produces_an_empty_order_not_a_guess():
    forecast = LeMansPredictor().predict(LeMansDataSource(), config.DEFAULT_SEASON, 1)
...
def test_strict_mode_fails_loudly_instead_of_returning_empty():
    ...
    with pytest.raises(snapshot.SnapshotUnavailable) as excinfo:
        LeMansDataSource(strict=True).calendar(config.DEFAULT_SEASON)
...
def test_fit_only_sees_prior_rounds():
    predictor = LeMansPredictor()
    predictor.fit(LeMansDataSource(), config.DEFAULT_SEASON, upto_round=5)
