...
def sample_finishing_orders(
    ...
    Same engine as :func:`plackett_luce_probabilities`, but returns the raw
    sampled orders (each a list of competitor codes, winner first) instead of
    collapsing them to market probabilities. This is the reusable primitive the
    championship Monte Carlo builds on — points are awarded per sampled order.

    ``values`` is a mapping ``competitor -> score`` where *lower is better*
    (lap time, predicted finishing metric, etc.), matching the rest of the
    calibration API.
    """
    if not values:
        raise ValueError("values is empty")
    competitors, strengths = _compute_strengths(values, temperature=temperature)
    rng = np.random.default_rng(seed=seed)
    rankings = _sample_rankings(strengths, n_samples=n_samples, rng=rng)
    return [[competitors[i] for i in row] for row in rankings]
...
# brier_score lives in `reliability` (the calibration-assessment module) and is
# re-exported here so `from motorsport_core.calibration import brier_score` keeps
# working — single implementation, no divergence.
from .reliability import brier_score  # noqa: E402,F401
