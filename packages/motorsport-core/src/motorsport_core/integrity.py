...
from .evidence import BASELINE_LABELS
...
def _season_level_baselines(directory: Path) -> set[str]:
    """Baseline names published once for the season rather than per round.

    The endurance products (WEC, IMSA) and the rally product score a class or a
    season, not a grid, so they publish `overall.lastRace` / `overall.seasonForm`
    and a per-class breakdown in season.json instead of a `baselines` block on
    every round file. The rule being enforced is that an accuracy number must
    have something to be compared against — not that the comparison lives in a
    particular file. Checking only the round files failed WEC and IMSA for a
    baseline they do publish, which would have taught the reader to ignore the
    gate rather than fix the data.
    """
    season = directory / "season.json"
    if not season.exists():
        return set()
    payload = _read_json(season)
    ...
    def walk(node: object, depth: int = 0) -> None:
        ...
        for key, value in node.items():
            if key in BASELINE_LABELS and isinstance(value, Mapping) and value:
                found.add(str(key))
            else:
                walk(value, depth + 1)
...
def check_baselines(directory: Path, root: Path) -> list[Finding]:
    ...
    if not pairs:
        return []
    season_baselines = _season_level_baselines(directory)
    missing: list[int] = []
    for number, path in pairs:
        payload = _read_json(path)
        if not isinstance(payload, dict):
    ...
    if missing and not season_baselines:
        return [
            Finding(
                "baselines_published",
                False,
    ...
    if missing:
        return [
            Finding(
                "baselines_published",
                True,
                f"baselines published for the season rather than per round "
                f"({', '.join(sorted(season_baselines))})",
                _rel(directory, root),
            )
        ]
    return [
