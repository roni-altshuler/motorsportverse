...
BASELINE_LABELS = {
    ...
    # The endurance and rally products score a season rather than a grid, so
    # their baselines are standings-shaped rather than last-race-shaped.
    "seasonForm": "Season form order",
    "standingsOrder": "Standings order",
    "championshipOrder": "Championship order",
    "classForm": "Class form order",
    # The flagship predates the shared naming and publishes snake_case. Its
    # keys are aliased rather than renamed: rewriting them would rewrite every
    # historical round file it has already published, and the published record
    # is the one thing in this repo that does not get retconned.
    "last_race_winner": "Last race order",
    "grid_order": "Grid order",
    "season_form": "Season form order",
    "standings_order": "Standings order",
    "lastRally": "Last rally order",
}
...
def _race_type_blocks(payload: Mapping) -> dict[str, Mapping]:
    ...
    The F1 flagship is the third shape: a single-race series that writes its
    metrics at the TOP level of the round file rather than nesting them under a
    race-type key. Nesting only earns its keep when a weekend has more than one
    scored session. Without this fallback the flagship — the site the whole
    design is modelled on — would publish an evidence panel with nothing in it.
    """
    nested = {
        key: value
    ...
    if nested:
        return nested
    if "mean_position_error" in payload:
        return {"race": payload}
    return {}


def _scored_scopes(payload: Mapping) -> list[tuple[dict[str, Mapping], dict[str, dict[str, Mapping]]]]:
    """``(model_blocks, baseline_blocks)`` pairs for one round file.

    Most series score a round once, so there is a single scope. The endurance
    products (WEC, IMSA) score a round once PER CLASS — a Hypercar forecast and
    an LMGT3 forecast are different predictions over different fields, and each
    class carries its own baselines inside its own entry. Each class therefore
    becomes its own scope, and its class name becomes the race type, so the
    comparison is paired within a class and never averaged across them.

    Averaging a Hypercar error against an LMGT3 error would produce a number
    that describes no actual forecast — the same category error as comparing
    mean position error between two different series.
    """
    ...
    if isinstance(classes, list) and classes:
        ...
        for entry in classes:
            if not isinstance(entry, Mapping):
                continue
            name = entry.get("class") or entry.get("label")
            metrics = _metric_block(entry)
            if not name or metrics is None:
                continue
            blocks = {str(name): metrics}
            scopes.append((blocks, _baseline_blocks(entry, blocks)))
        if scopes:
            return scopes
    race_blocks = _race_type_blocks(payload)
    return [(race_blocks, _baseline_blocks(payload, race_blocks))]


def _metric_block(node: object) -> Mapping | None:
    """The mapping carrying the metrics, unwrapping a ``score`` container.

    MotoGP and WRC publish ``{"score": {...}, "markets": {...}}`` where the
    older series publish the metrics flat. Unwrapping here keeps that detail
    out of every caller.
    """
    if not isinstance(node, Mapping):
        return None
    if "mean_position_error" in node:
        return node
    score = node.get("score")
    if isinstance(score, Mapping) and "mean_position_error" in score:
        return score
    return None
...
def _baseline_blocks(payload: Mapping, race_types: Iterable[str]) -> dict[str, dict[str, Mapping]]:
    ...
    Four published shapes reach this function, because four generations of the
    template wrote baselines differently. All normalise to the same nested map
    so the comparison code below sees one thing:

      feeder      baselines.sprint / baselines.feature      (implicit lastRace)
      single-race baselines.lastRace / baselines.gridOrder
      flagship    baselines.last_race_winner / .grid_order   (snake_case)
      nested      baselines.gridOrder.sprint.score / baselines.standingsOrder.score

    The last is MotoGP and WRC, which wrap metrics in a ``score`` sibling of a
    ``markets`` block. Matching on the metric rather than on the key name is
    what lets one function serve all four without a per-series lookup table.
    """
    ...
    for key, value in raw.items():
        if not isinstance(value, dict):
            continue
        direct = _metric_block(value)
        if key in known and direct is not None:
            # feeder shape: baselines.sprint / baselines.feature
            out.setdefault("lastRace", {})[key] = direct
            continue
        if direct is not None:
            # name-keyed, one block: baselines.lastRace / baselines.grid_order
            out.setdefault(key, {})[primary] = direct
            continue
        # name-keyed, per race type: baselines.gridOrder.sprint(.score)
        for race_type, sub in value.items():
            metrics = _metric_block(sub)
            if metrics is not None and race_type in known:
                out.setdefault(key, {})[race_type] = metrics
    return out
...
def build_evidence(
    ...
    *,
    ...
    for path in files:
        ...
        for race_blocks, baselines in _scored_scopes(payload):
